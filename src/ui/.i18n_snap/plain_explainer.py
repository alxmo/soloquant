...
class PlainExplainer:
    ...
    def explain_backtest(self, result: BacktestResult) -> str:
        ...
        lines.append(t('评级: {v1}', v1=grade_map.get(grade_val, '未知'))        lines.append("")

        # 年化收益
        ar = result.annual_return
        if ar > 0.20:
            lines.append(t('💰 每年平均赚 {v1:.0%}，非常厉害！', v1=ar)        elif ar > 0.10:
            lines.append(t('💰 每年平均赚 {v1:.0%}，比存银行好不少', v1=ar)        elif ar > 0.03:
            lines.append(t('💰 每年平均赚 {v1:.0%}，勉强跑赢银行存款', v1=ar)        else:
            lines.append(t('💰 每年平均赚 {v1:.0%}，不太理想', v1=ar)
        # 最大回撤
        mdd = result.max_drawdown
        if mdd < 0.05:
            lines.append(t('📉 最惨的时候只亏了 {v1:.0%}，很安全', v1=mdd)        elif mdd < 0.15:
            lines.append(t('📉 最惨的时候亏了 {v1:.0%}，可以接受', v1=mdd)        else:
            lines.append(t('📉 最惨的时候亏了 {v1:.0%}，需要心理准备', v1=mdd)
        # 夏普
        sr = result.sharpe_ratio
        if sr > 1.5:
            lines.append(t('📊 风险控制能力: 优秀 (夏普 {v1:.2f})', v1=sr)        elif sr > 1.0:
            lines.append(t('📊 风险控制能力: 不错 (夏普 {v1:.2f})', v1=sr)        elif sr > 0.5:
            lines.append(t('📊 风险控制能力: 一般 (夏普 {v1:.2f})', v1=sr)        else:
            lines.append(t('📊 风险控制能力: 需改善 (夏普 {v1:.2f})', v1=sr)
        # 胜率
        wr = result.win_rate
        if wr > 0.6:
            lines.append(t('🎯 每10次交易赢 {v1:.0f} 次，胜率较高', v1=wr*10)        elif wr > 0.5:
            lines.append(t('🎯 每10次交易赢 {v1:.0f} 次，还行', v1=wr*10)        else:
    ...
    def explain_signal(self, signal: TradeSignal) -> str:
        ...
        if signal.direction.value == "buy":
            if signal.target_weight > 0.20:
                parts.append(t('重仓 ({v1})，说明模型很看好', v1=weight)            elif signal.target_weight > 0.10:
                parts.append(t('中等仓位 ({v1})', v1=weight)            else:
                parts.append(t('小仓位 ({v1})，先试试水', v1=weight)        else:
    ...
    def explain_risk(self, risk: RiskStatus) -> str:
        ...
        if not risk.is_alert:
            ...
            lines.append(t('  ⚠️ 有 {v1} 条警报:', v1=len(risk.alerts))            for alert in risk.alerts:
        ...
        if dd > 0:
            if dd < 0.05:
                lines.append(t('  当前回撤 {v1:.1%}，很安全', v1=dd)            elif dd < 0.10:
                lines.append(t('  当前回撤 {v1:.1%}，需要注意', v1=dd)            elif dd < 0.15:
                lines.append(t('  当前回撤 {v1:.1%}，接近警戒线！', v1=dd)            else:
    ...
    def explain_account(self, account: Account) -> str:
        """通俗解读账户状态"""
        lines = []
        lines.append(t('💰 你的账户:')        lines.append(t('  总资产: ${v1:,.2f}', v1=account.equity)
        mode = t("📝 模拟盘") if account.trading_mode.value == "paper" else t("🔴 实盘")
        lines.append(t('  交易模式: {v1}', v1=mode)        lines.append(t('  可用现金: ${v1:,.2f}', v1=account.cash)        lines.append(t('  购买力: ${v1:,.2f}', v1=account.buying_power)
        # 日盈亏
    ...
    def explain_position(self, position: Position) -> str:
        ...
        lines.append(t('  {v1}: {v2:.2f}股 | {v3} ${v4:+,.2f} ({v5:+.1%}) | 占仓位 {v6:.1%}', v1=position.symbol, v2=position.qty, v3=pnl_icon, v4=position.unrealized_pnl, v5=position.unrealized_pnl_pct, v6=position.weight)        return "\n".join(lines)
