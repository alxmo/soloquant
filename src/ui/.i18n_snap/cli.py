...
def print_status():
    """打印系统状态"""
    print(f"{'─' * 60}")
    print(t('  📋 系统配置:')    print(t('     交易模式: {v1}', v1='📝 模拟盘 (Paper)' if config.is_paper_trading() else '🔴 实盘 (Live)')    print(f"     Alpaca:   {'✅ 已配置' if config.has_alpaca_keys() else '❌ 未配置'}")
    ...
    try:
        ...
        if llm_st["enabled"]:
            print(t('     LLM: 🧠 智能模式 (模型: {v1})', v1=llm_st['default_model'])        elif llm_st["total_models"] > 0:
            avail = llm_st["available_models"]
            if avail:
                print(t('     LLM: 🧠 智能模式 (模型: {v1})', v1=', '.join(avail))            else:
                print(t('     LLM: 📝 规则模式 (已配置 {v1} 个模型但Key未填)', v1=llm_st['total_models'])        else:
            print(t('     LLM: 📝 规则模式 (未配置)')    except Exception:
        print(t('     LLM: 📝 规则模式')
    print(f"{'─' * 60}")
...
def chat_mode():
    ...
    try:
        ...
        if llm_config.is_enabled():
            print(t('\n🧠 智能对话模式已启用 (LLM: {v1})', v1=llm_config.get_default_name())            print(t("   支持自然语言理解、多轮对话上下文记忆。"))
        else:
    ...
    while True:
        ...
        try:
            ...
            logger.error(t('对话处理异常: {v1}', v1=e)            print(t('\n🤖 抱歉，处理你的请求时出现了错误: {v1}', v1=e)            print(t("   请稍后重试，或输入 \"帮助\" 查看可用功能。"))
...
def _menu_full_auto():
    ...
    try:
        ...
        print(f"\n{'═' * 50}")
        print(t('  📋 全自动循环完成!')        print(t('  步骤: {v1}', v1=result.get('steps', {}))
        # 打印各 Agent 报告
        for step_name in ["research", "strategy", "backtest", "execution", "monitor"]:
            ...
            if agent and agent.last_result() is not None:
                print(t('\n📋 {v1}报告:', v1=agent.role)                print(agent.plain_explain())
    except Exception as e:
...
def _menu_emergency_stop():
    ...
    try:
        ...
        print(f"\n{'═' * 50}")
        print(t('  📋 紧急止损完成!')        print(t('  步骤: {v1}', v1=result.get('steps', {}))
        # 打印各步骤报告
        step_agent_map = {
            "risk_check": "monitor",
            "liquidate": "execution",
...
def _menu_init_qlib():
    ...
    print(t('\n  📦 正在初始化 Qlib 数据: {v1}', v1=symbols)    from src.data.qlib_adapter import qlib_adapter
    result = qlib_adapter.init_qlib_data(symbols)

    print(t('  ✅ 成功: {v1}', v1=result['success'])    print(t('  失败: {v1}', v1=result['failed'])
...
def _launch_dashboard():
    ...
    if _is_port_in_use(8502):
        print(t('  ⚠️ 端口 8502 已被占用, 跳过聊天服务启动 (可能已在运行)')    else:
        try:
            if is_frozen:
                ...
                chat_thread.start()
                print(t('  🤖 智能助手聊天服务已启动 (线程模式, 端口 8502)')            else:
                ...
                print(t('  🤖 智能助手聊天服务已启动 (PID: {v1}, 端口 8502)', v1=chat_proc.pid)        except Exception as e:
    ...
    try:
        ...
        print(t('  ❌ 仪表盘启动失败: {v1}', v1=e)    except KeyboardInterrupt:
        print(t("\n  仪表盘已退出。"))
    finally:
        # 退出时清理聊天服务
...
def _run_streamlit_frozen():
    ...
    if not os.path.exists(dashboard_path):
        print(t('  ❌ 找不到仪表盘文件: {v1}', v1=dashboard_path)        print(t("     请确保打包时已包含 src/ui/dashboard.py"))
        return
    ...
    try:
        ...
        try:
            ...
            print(t('  ❌ Streamlit 启动失败: {v1}', v1=e)            print(t("     请确保 streamlit 已正确打包"))
    except Exception as e:
        print(t('  ❌ Streamlit 启动失败: {v1}', v1=e)    finally:
...
def _launch_api():
    ...
    print(t('  📡 监听: http://{v1}:{v2}', v1=host, v2=port)    print(t('  📋 API 文档: http://{v1}:{v2}/docs', v1=host, v2=port)    print(f"  🔑 API Key: {'已配置' if config.MODEL_API_KEY else '未配置 (无认证)'}")
    print(t("  按 Ctrl+C 停止\n"))
...
def _launch_cloud():
    ...
    print(t('  📡 API 端口: {v1}', v1=port)    print(f"  📊 Dashboard: 8501")
    print(t('  ⏰ 调度器: 自动注册默认任务')    print(t("  按 Ctrl+C 停止所有服务\n"))
            ...
            try:
                def _run_dashboard_cloud():
                    try:
                        ...
                        print(t('  ⚠️ Dashboard 异常: {v1}', v1=e)                dashboard_thread = threading.Thread(target=_run_dashboard_cloud, daemon=True)
            ...
            try:
                def _run_chat_cloud():
                    try:
                        ...
                        print(t('  ⚠️ 聊天服务异常: {v1}', v1=e)                chat_thread = threading.Thread(target=_run_chat_cloud, daemon=True)
                chat_thread.start()
                print(t("  ✅ 智能助手聊天服务已启动 (线程模式)"))
            except Exception as e:
                print(t('  ⚠️ 智能助手聊天服务启动失败: {v1}', v1=e)    else:
        # 开发模式: 用 subprocess 启动独立进程
        # 检查 8501 是否已占用
        if _is_port_in_use(8501):
            print(t("  ⚠️ 端口 8501 已被占用, 跳过 Dashboard 启动 (可能已在运行)"))
            ...
            try:
                ...
                print(t('  ✅ Dashboard 已启动 (PID: {v1})', v1=dashboard_proc.pid)            except Exception as e:
                print(t('  ⚠️ Dashboard 启动失败: {v1}', v1=e)
        # 检查 8502 是否已占用
        if _is_port_in_use(8502):
            print(t("  ⚠️ 端口 8502 已被占用, 跳过聊天服务启动 (可能已在运行)"))
            ...
            try:
                ...
                print(t('  ✅ 智能助手聊天服务已启动 (PID: {v1})', v1=chat_proc.pid)            except Exception as e:
                print(t('  ⚠️ 智能助手聊天服务启动失败: {v1}', v1=e)
    # 启动 API (主进程阻塞)
    try:
        import uvicorn
...
def _launch_realtime(symbols=None):
    ...
    print(t('  📋 订阅股票: {v1}', v1=', '.join(symbols))    print(t('  📡 数据源: Alpaca {v1} | Finnhub {v2}', v1='✅' if alpaca_ok else '❌', v2='✅' if finnhub_ok else '❌')    print(t('  🔧 数据源模式: {v1}', v1=config.REALTIME_DATA_SOURCE)    print(t('  🚨 价格异动告警阈值: {v1:.1%}', v1=config.REALTIME_PRICE_ALERT_THRESHOLD)    print(t("  按 Ctrl+C 停止\n"))
    ...
    async def _run():
        ...
        while realtime_manager.is_running:
            ...
            print(t('\n  ── 状态 [{v1}] ──', v1=datetime.now().strftime('%H:%M:%S'))            print(t('  缓存价格: {v1} 只 | 告警: {v2} 条', v1=status['cached_prices'], v2=status['alerts_count'])            prices = realtime_manager.get_all_prices()
...
def _quick_health_check():
    ...
    try:
        ...
        if df is not None and len(df) > 0:
            print(t('  ✅ AKShare 数据获取正常 (AAPL: {v1} 条)', v1=len(df))        else:
    ...
    if config.has_finnhub_key():
        try:
            ...
            print(t('  ✅ Finnhub API 正常')        except Exception as e:
            print(t('  ⚠️ Finnhub API 异常: {v1}', v1=e)    else:
    ...
    if config.has_alpaca_keys():
        try:
            ...
            if account:
                print(t('  ✅ Alpaca 连接正常 (Paper: ${v1:,.2f})', v1=account.cash)            else:
                print(t("  ❌ Alpaca 连接失败"))
        except Exception as e:
            print(t('  ⚠️ Alpaca 异常: {v1}', v1=e)    else:
        print(t("  ⚠️ Alpaca API Key 未配置"))

    # 检查 Qlib
    if config.QLIB_DATA_PATH.exists():
        print(t('  ✅ Qlib 数据目录: {v1}', v1=config.QLIB_DATA_PATH)    else:
    ...
    try:
        ...
        llm_st = llm_config.status()
        if llm_st["enabled"]:
            print(t('  ✅ LLM 已启用 (默认模型: {v1})', v1=llm_st['default_model'])            for m in llm_config.get_available_models():
                print(f"     • {m.name}: {m.model_id} @ {m.endpoint}")
        elif llm_st["total_models"] > 0:
            print(t('  ⚠️ 已配置 {v1} 个模型但 Key 未填写', v1=llm_st['total_models'])            print(t('     模型列表: {v1}', v1=', '.join(llm_st['all_models']))            print(t('     请在 .env 中填入对应的 API Key')        else:
            print(t('  ℹ️ 未配置 LLM (使用规则引擎模式)')    except Exception as e:
    ...
    if len(sys.argv) > 1:
        ...
        if cmd == "chat":
            ...
            try:
                ...
                print(t('\n全自动循环完成: {v1}', v1=result.get('steps', {}))            except Exception as e:
                print(t('\n❌ 全自动循环失败: {v1}', v1=e)            return
        elif cmd == "goal":
            # 目标驱动 AI 自主交易
            ...
            print(t('初始化完成: {v1}', v1=result)            return
        elif cmd == "emergency":
            # 命令行直接执行紧急止损
            try:
                result = workflow_engine.run_emergency_stop()
                print(t('\n紧急止损完成: {v1}', v1=result.get('steps', {}))            except Exception as e:
                print(t('\n❌ 紧急止损失败: {v1}', v1=e)            return
        elif cmd == "dashboard":
            # 启动 Streamlit 仪表盘
            _launch_dashboard()
            return
