...
class NLRouter:
    ...
    def _respond_with_llm(self, text: str, page_context: str = "") -> str:
        ...
        try:
            ...
            logger.error(t('Function Agent 执行异常: {v1}', v1=e)fo=True)
    ...
    def _route_intent(
        ...
        try:
            ...
            logger.error(t('意图路由异常 [{v1}]: {v2}', v1=intent, v2=e)            return (None, "", str(e))
    ...
    def _exec_goal_driven(self, text: str, params: dict) -> tuple:
        ...
        try:
            ...
            logger.error(t('目标驱动交易失败: {v1}', v1=e)            return (None, t("AI自主交易"), f"执行失败: t('执行失败: {v1}', v1=e)
    @staticmethod
    def _format_workflow_result(result: dict, name: str) -> str:
        """格式化工作流结果为文本摘要"""
        lines = [t('✅ {v1}完成!', v1=name)        steps = result.get("steps", {})
    ...
    def _handle_research(self, text: str) -> str:
        ...
        try:
            ...
            logger.error(t('调研失败: {v1}', v1=e)            return t('❌ 调研过程中出现错误: {v1}\n\n请稍后重试。', v1=e)
    def _handle_strategy(self, text: str) -> str:
        ...
        try:
            ...
            logger.error(t('策略生成失败: {v1}', v1=e)            return t('❌ 策略生成过程中出现错误: {v1}', v1=e)
    def _handle_backtest(self, text: str) -> str:
        ...
        try:
            ...
            logger.error(t('回测失败: {v1}', v1=e)            return t('❌ 回测过程中出现错误: {v1}', v1=e)
    def _handle_execute(self, text: str) -> str:
        ...
        try:
            ...
            logger.error(t('交易执行失败: {v1}', v1=e)            return t('❌ 交易执行过程中出现错误: {v1}', v1=e)
    def _handle_monitor(self, text: str) -> str:
        ...
        try:
            ...
            logger.error(t('监控失败: {v1}', v1=e)            return t('❌ 监控过程中出现错误: {v1}', v1=e)
    ...
    def _status_text(self) -> str:
        ...
        lines.append(t("📊 系统状态"))
        lines.append(f"{'─' * 40}")
        lines.append(t('  交易模式: {v1}', v1='📝 模拟盘 (Paper)' if config.is_paper_trading() else '🔴 实盘 (Live)')        lines.append(f"  Alpaca:   {'✅ 已配置' if config.has_alpaca_keys() else '❌ 未配置'}")
        lines.append(f"  Finnhub:  {'✅ 已配置' if config.has_finnhub_key() else '❌ 未配置'}")
        lines.append(t('  Qlib数据: {v1}', v1='✅ 已初始化' if config.QLIB_DATA_PATH.exists() else '⚠️ 未初始化')        lines.append(t('  最大仓位: {v1:.0%}', v1=config.MAX_POSITION_PCT)        lines.append(t('  日最大亏损: {v1:.0%}', v1=config.MAX_DAILY_LOSS)        lines.append(t('  最大回撤: {v1:.0%}', v1=config.MAX_DRAWDOWN)
        # LLM 状态 (v0.4.0)
        llm_status = llm_config.status()
        if llm_status["enabled"]:
            lines.append(t('  LLM: 🧠 智能模式 (模型: {v1})', v1=llm_status['default_model'])        elif llm_status["total_models"] > 0:
            lines.append(t('  LLM: 📝 规则模式 (已配置但Key未填)')        else:
    ...
    def _handle_full_loop(self, text: str) -> str:
        ...
        try:
            ...
            lines.append(t('✅ 全自动循环完成!')            lines.append("")
            ...
            for step_name, step_result in result.get("results", {}).items():
                ...
                if agent and agent.last_result() is not None:
                    lines.append(t('📋 {v1}报告:', v1=agent.role)                    lines.append(agent.plain_explain())
                    lines.append("")
    ...
    def _handle_stock_screen(self, text: str) -> str:
        ...
        try:
            ...
            logger.error(t('选股失败: {v1}', v1=e)            return t('❌ 选股过程中出现错误: {v1}', v1=e)
    def _handle_goal_driven(self, text: str) -> str:
        ...
        try:
            ...
            logger.error(t('目标驱动交易失败: {v1}', v1=e)            return t('❌ AI 自主交易执行失败: {v1}\n\n请稍后重试。', v1=e)
    def _handle_emergency(self, text: str) -> str:
        ...
        try:
            ...
            lines.append(t('✅ 紧急止损完成!')            lines.append("")
    ...
    def _respond_stream_with_llm(self, text: str, page_context: str = ""):
        ...
        try:
            ...
            self._history[-1]["response"] = t('异常: {v1}', v1=e)            self._history[-1]["llm_mode"] = True
            return
    ...
    def _execute_chain_stream(self, chain: LLMIntentChain, text: str, page_context: str = ""):
        ...
        for idx, llm_intent in enumerate(chain.intents, 1):
            ...
            try:
                ...
                logger.warning(t('链式步骤 {v1} 回复生成失败: {v2}', v1=idx, v2=e)                reply = self._generate_fallback_response(
                    intent, agent_result, agent_role, error, text
                )

            # 步骤标题
            if total > 1:
                step_header = t('**步骤 {v1}/{v2}: {v3}**\n\n', v1=idx, v2=total, v3=label)                reply = step_header + reply
    ...
    def load_session(self, messages: list) -> None:
        ...
        try:
            ...
            if restored:
                logger.info(t('会话上下文已恢复 ({v1} 条消息)', v1=restored)        except Exception as e:
