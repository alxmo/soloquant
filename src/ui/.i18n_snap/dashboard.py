    ...
    try:
        ...
        if data is None:
            ...
            if isinstance(err, dict) and "error" in err:
                st.session_state["_account_error"] = t('账户获取失败: {v1}', v1=err['error'])            else:
...
def get_macro_dashboard_data():
    ...
    try:
        ...
        st.error(t('获取宏观数据失败: {v1}', v1=e)        return {}
...
def get_yield_curve_data():
    ...
    try:
        ...
        st.error(t('获取收益率曲线失败: {v1}', v1=e)        return {}
...
def get_options_chain_data(symbol: str, expiration_date: str = ""):
    ...
    try:
        ...
        st.error(t('获取期权链失败: {v1}', v1=e)        return {}
...
def render_sidebar():
    ...
    with st.sidebar.expander(t("🔌 数据源切换"), expanded=False):
        ...
        if new_mode == "cloud":
            ...
            if api_base:
                st.caption(t('📡 连接: `{v1}`', v1=api_base)                # 快速健康检查
                try:
                    cc = _get_cloud_client()
...
def _render_realtime_panel():
    ...
    try:
        ...
        with ctrl_col3:
            st.metric(t("缓存价格"), f"{statut('{v1} 只', v1=status['cached_prices'])        with ctrl_col4:
            st.metric(t("告警数"), f"{stat('{v1} 条', v1=status['alerts_count'])
        # 数据源状态
...
def page_overview():
    ...
    if account:
        ...
        col4.metric(t("📋 交易模式"), account["trading_mode"])
    else:
        _err = st.session_state.get("_account_error", "")
        if _err:
            st.warning(t('⚠️ 账户数据获取失败: {v1}', v1=_err)        else:
...
def page_stock_screen():
    ...
    with st.expander(t("⚙️ 手动调整参数 (可选)"), expanded=False, key="expander_manual_screen_params"):
        ...
        with col3:
            screen_keywords = st.text_input(t("关键词"), "潜力", key="screen_keywords", help=t("如: 潜力/成长/低价"))
    ...
    if st.button(t("🔍 开始选股"), type="primary", key="btn_screen"):
        ...
        if nl_input.strip():
            ...
            try:
                ...
                if llm_intent.intent == "stock_screen":
                    ...
                    st.info(t('🧠 AI 解析结果: 数量={v1}, 价格={v2}~{v3}, 行业={v4}, 关键词={v5}', v1=params.get('count', 5), v2=params.get('price_min', 0), v3=params.get('price_max', '不限'), v4=params.get('sector', '不限'), v5=params.get('keywords', '无'))                else:
                    st.warning(t('未识别为选股意图 (识别为: {v1})，使用手动参数', v1=llm_intent.intent)            except Exception as e:
        ...
        with st.spinner(t("🔍 正在扫描全美股并评分... (可能需要 30-60 秒)")):
            try:
                ...
                st.error(t('选股失败: {v1}', v1=e)                return
    ...
    if result:
        ...
        if len(result.picks) > 0:
            ...
            with col_radar:
                ...
                fig_radar.update_layout(
                    ...
                    title=t('{v1} 四维评分雷达图', v1=pick.symbol)                    height=350,
                )
            ...
            # ─── 公司简介 ───
            with st.spinner(t('正在加载 {v1} 公司信息...', v1=pick.symbol)                try:
                    from src.data.collector import data_collector
                    profile = None
            ...
            with col_action:
                ...
                with col_watch1:
                    if is_watched:
                        if st.button(t("💔 取消关注"), key=f"btn_unwatch_{pick.symbol}", use_container_width=True):
                            user_watchlist.remove(pick.symbol)
                            _save_user_watchlist(user_watchlist)
                            st.success(t('已取消关注 {v1}', v1=pick.symbol)                            st.rerun()
                    else:
                        if st.button(t("⭐ 关注该股票"), key=f"btn_watch_{pick.symbol}", use_container_width=True):
                            if pick.symbol not in user_watchlist:
                                user_watchlist.append(pick.symbol)
                                _save_user_watchlist(user_watchlist)
                            st.success(t('已关注 {v1}', v1=pick.symbol)                            st.rerun()
                with col_watch2:
                ...
                if user_watchlist:
                    ...
                    if len(user_watchlist) > 15:
                        watch_chips += t(' ...等 {v1} 只', v1=len(user_watchlist))                    st.caption(t('📋 关注列表: {v1}', v1=watch_chips)
                st.divider()
                ...
                with st.expander(t("为该股票创建策略"), expanded=False, key=f"expander_strategy_{pick.symbol}"):
                    # 策略名称 (默认值包含股票代码和日期)
                    default_name = t('选股策略_{v1}_{v2}', v1=pick.symbol, v2=datetime.now().strftime('%Y%m%d'))                    strategy_name = st.text_input(
                        t("策略名称"),
                        value=default_name,
                    ...
                    if st.button(t("🚀 创建策略"), type="primary", key=f"btn_create_strat_{pick.symbol}", use_container_width=True):
                        try:
                            ...
                            st.success(t("✅ 策略 '{v1}' 创建成功! (ID: {v2})", v1=strategy.name, v2=strategy_id)                            st.info(t('💡 股票池: {v1} | 模型: {v2} | 仓位: {v3:.0%}', v1=pick.symbol, v2=strategy.model_type, v3=max_position)                            st.caption(t("👉 前往「📈 策略管理」查看详情和回测"))
                        except Exception as e:
            ...
            # ════════════════════════════════════════
            st.divider()
            with st.expander(t('📊 {v1} K线图', v1=pick.symbol)ded=True, key=f"expander_kline_{pick.symbol}"):
                # K线参数选择
                ...
                with st.spinner(t('正在加载 {v1} K线数据...', v1=pick.symbol)                    try:
                        ...
                        if kline_df is not None and len(kline_df) > 0:
                            ...
                            # 显示最近几日K线数据摘要
                            st.caption(t('📅 数据范围: {v1} ~ {v2} | 共 {v3} 根K线', v1=kline_df['datetime'].iloc[0].strftime('%Y-%m-%d'), v2=kline_df['datetime'].iloc[-1].strftime('%Y-%m-%d'), v3=len(kline_df))                        else:
                            st.warning(t('暂无 {v1} 的K线数据，请检查数据源', v1=pick.symbol)                    except Exception as e:
        ...
        overview_data = {
            ...
            t("详情"): [
                t('{v1}, 选 {v2} 只', v1=_cond, v2=_count)                str(result.total_scanned),
...
def page_strategies():
    ...
    if templates:
        ...
        for i, tmpl in enumerate(templates):
            ...
            with col:
                ...
                st.text(t('模型: {v1} | 因子: {v2}个', v1=tmpl['model_type'], v2=tmpl['factors_count'])                st.text(t('股票池: {v1}只 | 仓位: {v2:.0%}', v1=tmpl['stock_pool_size'], v2=tmpl['max_position'])                tags_str = " ".join(f"#{t}" for t in tmpl["tags"][:3])
                st.caption(tags_str)
                if st.button(t('创建')y=f"btn_tmpl_{tmpl['template_id']}"):
                    try:
                        strategy = strategy_templates.create_from_template(tmpl["template_id"])
                        if strategy:
                            st.success(t("✅ 策略 '{v1}' 创建成功! (ID: {v2})", v1=strategy.name, v2=strategy.strategy_id)                            st.rerun()
                    except Exception as e:
                        st.error(t('创建失败: {v1}', v1=e)
    st.divider()
    ...
    st.subheader(t('📋 策略列表 ({v1} 个)', v1=len(strategies))    # 将策略数据转为扁平 dict (兼容对象和 dict 两种格式)
    strat_rows = []
    for s in strategies:
            ...
            if st.button(t("创建组合"), key="btn_create_portfolio"):
                if len(selected_sids) < 2:
                    ...
                    try:
                        ...
                        st.success(t("✅ 组合 '{v1}' 创建成功! (ID: {v2})", v1=p.name, v2=p.portfolio_id)                        st.rerun()
                    except Exception as e:
                        st.error(t('创建失败: {v1}', v1=e)
        # 列出现有组合
                    ...
                    with col_r1:
                        if st.button(t("🔄 重新分配权重"), key=f"btn_realloc_{p.portfolio_id}"):
                            try:
                                ...
                                st.success(t('✅ 权重已更新: {v1}', v1=new_weights)                                st.rerun()
                            except Exception as e:
                                st.error(t('分配失败: {v1}', v1=e)                    with col_r2:
...
def page_research():
    ...
    if st.button(t("🔍 调研指定股票"), type="secondary", key="btn_research_single"):
        if not stock_input.strip():
            ...
            try:
                ...
                research_agent = registry.get("research")
                if research_agent:
                    with st.spinner(t('正在深度调研 {v1} 只股票...', v1=len(symbols))                        single_results = []
                        for sym in symbols:
                            result = research_agent.research_single_stock(sym)
                            single_results.append(result)
                        st.session_state["research_single_results"] = single_results
                    st.success(t('✅ {v1} 只股票调研完成!', v1=len(single_results))                else:
                    st.error(t("Research Agent 未就绪"))
            except Exception as e:
                st.error(t('调研失败: {v1}', v1=e)
    st.divider()
    ...
    if st.button(t("⭐ 调研全部关注股票"), type="secondary", key="btn_research_watchlist"):
        try:
            ...
            research_agent = registry.get("research")
            if research_agent:
                with st.spinner(t('正在调研 {v1} 只关注股票... (可能需要30-60秒)', v1=len(watchlist))                    results = research_agent.research_watchlist()
                    st.session_state["research_watchlist_results"] = results
                st.success(t('✅ {v1} 只关注股票调研完成!', v1=len(results))            else:
                st.error(t("Research Agent 未就绪"))
        except Exception as e:
    ...
    if single_results:
        st.subheader(t('🔎 单股调研结果 ({v1} 只)', v1=len(single_results))        for r in single_results:
    ...
    if watchlist_results:
        st.subheader(t('⭐ 关注股票调研结果 ({v1} 只)', v1=len(watchlist_results))        _render_watchlist_research(watchlist_results)
        st.divider()
...
def _render_single_stock_research(r: dict):
    ...
    with st.expander(
        t('{v1} - {v2} | {v3} 情绪: {v4} ({v5:+.2f})', v1=r['symbol'], v2=r['name'], v3=sent_icon, v4=r.get('sentiment_label', '中性'), v5=sentiment)        expanded=True,
    ):
        col1, col2, col3, col4 = st.columns(4)
...
def page_macro():
    ...
    if risk_score:
        ...
        level = risk_score.get("level", t("未知"))
        ...
        st.info(t('💡 建议: {v1}', v1=recommendation)
        if signals:
            st.markdown(t("**风险信号:**"))
            for sig in signals:
                st.text(f"  {sig}")
    ...
    for category, label in category_names.items():
        ...
        with st.expander(t('{v1} ({v2} 个指标)', v1=label, v2=len(items))nded=True):
            rows = []
            for item in items:
    ...
    if yield_curve:
        ...
        # 倒挂警告
        if is_inverted:
            st.error(t('🚨 {v1} - 收益率倒挂是经济衰退的领先指标!', v1=inversion_signal)        else:
    ...
    try:
        ...
        if vix_data:
            ...
            col_v4.metric(t("风险等级"), vix_data["risk_level"])

            color_map = {"green": "🟢", "lightgreen": "🟢", "yellow": "🟡", "orange": "🟠", "red": "🔴"}
            st.info(t('{v1} VIX 风险等级: {v2} | 数据时间: {v3}', v1=color_map.get(vix_data['risk_color'], '⚪'), v2=vix_data['risk_level'], v3=vix_data.get('timestamp', 'N/A'))        else:
...
def page_options():
    ...
    with st.spinner(t('正在获取 {v1} 期权链数据...', v1=symbol_input)        # 先获取一次以拿到到期日列表
        opt_data = get_options_chain_data(symbol_input, "")

    if not opt_data:
        st.error(t('获取 {v1} 期权链失败，可能该股票没有期权数据', v1=symbol_input)        return
    ...
    if selected_exp and selected_exp != opt_data.get("selected_expiration"):
        with st.spinner(t('正在获取 {v1} 到期日期权数据...', v1=selected_exp)            opt_data = get_options_chain_data(symbol_input, selected_exp)
    ...
    if isinstance(calls_df, pd.DataFrame) and not calls_df.empty:
        ...
        if available_call_cols:
            ...
            st.subheader(t('📈 看涨期权 - {v1} ({v2} 个合约)', v1=selected_exp, v2=len(display_calls))            st.dataframe(display_calls, width='stretch', hide_index=True)
    ...
    if isinstance(puts_df, pd.DataFrame) and not puts_df.empty:
        ...
        if available_put_cols:
            ...
            st.subheader(t('📉 看跌期权 - {v1} ({v2} 个合约)', v1=selected_exp, v2=len(display_puts))            st.dataframe(display_puts, width='stretch', hide_index=True)
    ...
    if (isinstance(calls_df, pd.DataFrame) and not calls_df.empty and
        ...
        fig_smile.update_layout(
            title=t('{v1} 隐含波动率曲线 - {v2}', v1=symbol_input, v2=selected_exp)            xaxis_title=t("行权价"),
            yaxis_title=t("隐含波动率 (%)"),
            height=400,
        )
...
def page_crypto():
    ...
    try:
        ...
        if account_data:
            ...
            col2.metric(t("💵 现金"), f"")
            col3.metric(t("💪 购买力"), f"")
            mode_label = account_data.get("trading_mode", t("📝 模拟盘"))
            col4.metric(t("📋 交易模式"), mode_label)
    ...
    if price_results:
        ...
        col_info1.caption(t('✅ {v1} 个交易对可获取价格', v1=available_count)        if unavailable_count > 0:
    ...
    try:
        ...
        if not alpaca_client.available:
            ...
            with col_trade3:
                ...
                if current_price:
                    ...
                    st.caption(t('预估金额: ${v1:,.2f}', v1=est_value)                    # 最小订单金额校验提示
                    if est_value < 10.0:
                        st.warning(t('⚠️ 预估金额 ${v1:.2f} < $10, Alpaca 最小订单金额为 $10, 请增加数量', v1=est_value)                else:
                ...
                if st.button(t("📈 买入"), type="primary", key="btn_crypto_buy"):
                    if not selected_pair or trade_qty <= 0:
                        ...
                        if current_price and current_price * trade_qty < 10.0:
                            st.error(t('❌ 订单金额 ${v1:.2f} < $10 (Alpaca 最小限额), 请增加数量', v1=current_price * trade_qty)                        else:
                            with st.spinner(t('正在买入 {v1} {v2}...', v1=trade_qty, v2=selected_pair)                                try:
                                    ...
                                    if order:
                                        st.success(t('✅ 买入订单已提交! ID: {v1}', v1=order.order_id)                                        st.info(t('📊 {v1} × {v2:.4f} @ 市价', v1=selected_pair, v2=trade_qty)                                    else:
            ...
            with col_sell:
                if st.button(t("📉 卖出"), type="secondary", key="btn_crypto_sell"):
                    if not selected_pair or trade_qty <= 0:
                        ...
                        if current_price and current_price * trade_qty < 10.0:
                            st.error(t('❌ 订单金额 ${v1:.2f} < $10 (Alpaca 最小限额), 请增加数量', v1=current_price * trade_qty)                        else:
                            with st.spinner(t('正在卖出 {v1} {v2}...', v1=trade_qty, v2=selected_pair)                                try:
                                    order = alpaca_client.submit_crypto_order(
                                        symbol=selected_pair,
                                        qty=trade_qty,
                                        side="sell",
                                    ...
                                    if order:
                                        st.success(t('✅ 卖出订单已提交! ID: {v1}', v1=order.order_id)                                        st.info(t('📊 {v1} × {v2:.4f} @ 市价', v1=selected_pair, v2=trade_qty)                                    else:
                                        st.error(t("订单提交失败，请检查 API 配置"))
                                except Exception as e:
        ...
        st.warning(t('手动交易面板加载异常: {v1}', v1=e)        st.warning(t('获取价格数据异常: {v1}', v1=e)
    st.divider()
    ...
    col3.metric(t("Paper 最少天数"), f"{confit('{v1} 天', v1=config.CRYPTO_MIN_PAPER_DAYS)
    # 风控检查
    try:
            ...
            with st.spinner(t("正在执行加密货币交易循环...")):
                try:
                    ...
                    if result.get("status") == "success":
                        ...
                        st.warning(t('⚠️ 执行完成 (有异常): {v1}', v1=result.get('status'))                    # 显示执行摘要
...
def page_settings():
    ...
    if workflows:
        ...
        if st.button(t("▶️ 运行工作流"), type="primary"):
            with st.spinner(t('正在运行 {v1}...', v1=selected_wf)                try:
                    ...
                    if steps:
                        ...
                        for step_name, step_result in steps.items():
                            if step_result is not None:
                                st.text(t('  ✅ {v1}: 已完成', v1=step_name)                            else:
                                st.text(t('  ❌ {v1}: 失败或跳过', v1=step_name)                except Exception as e:
                    st.error(t('工作流执行失败: {v1}', v1=e)    else:
        st.info(t("工作流引擎未初始化"))
                ...
                try:
                    ...
                    if monitor:
                        ...
                        if isinstance(result, dict):
                            ...
                            if alerts:
                                st.warning(t('检测到 {v1} 条告警:', v1=len(alerts))                                for a in alerts:
                                    st.text(f"  ⚠️ {a}")
                            else:
            ...
            if st.button(t("✅ 确认清仓")):
                with st.spinner(t("正在执行紧急清仓...")):
                    try:
                        ...
                        st.error(t('紧急清仓失败: {v1}', v1=e)                st.session_state["confirm_emergency"] = False
        with col2:
            if st.button(t("❌ 取消")):
                        ...
                        with st.spinner(t("正在执行迁移...")):
                            try:
                                ...
                                if result.get("success"):
                                    ...
                                    st.error(t('迁移失败: {v1}', v1=result.get('reason', '未知'))                            except Exception as e:
                                st.error(t('迁移失败: {v1}', v1=e)                        st.session_state["confirm_migrate"] = False
                with col_c2:
                    if st.button(t("❌ 取消"), key="btn_cancel_migrate"):
                        st.session_state["confirm_migrate"] = False
                        st.rerun()
        else:
            st.info(t('📌 已迁移到实盘 | 迁移时间: {v1}', v1=mig_status.get('migration_date', 'N/A')[:19])            if st.button(t("📋 回滚到模拟盘"), type="secondary", key="btn_rollback"):
    ...
    try:
        ...
        if sched_status["running"]:
            st.success(t('调度器运行中 | {v1} 个任务', v1=sched_status['task_count'])        else:
            st.info(t("调度器未启动"))
        ...
        with col1:
            if st.button(t("▶️ 启动调度器")) and not sched_status["running"]:
                try:
                    ...
                    st.error(t('启动失败: {v1}', v1=e)        with col2:
    ...
    with st.expander(t("🔑 API 密钥"), expanded=False):
        ...
        new_alpaca_key = st.text_input(
            "ALPACA_API_KEY",
            value="",
            placeholder=t('当前: {v1}', v1=alpaca_key_display)alpaca_key_display else t("未配置"),
        ...
        new_alpaca_secret = st.text_input(
            "ALPACA_SECRET_KEY",
            value="",
            placeholder=t('当前: {v1}', v1=alpaca_secret_display)alpaca_secret_display else t("未配置"),
        ...
        new_finnhub_key = st.text_input(
            "FINNHUB_API_KEY",
            value="",
            placeholder=t('当前: {v1}', v1=finnhub_display)finnhub_display else t("未配置"),
        ...
        new_fred_key = st.text_input(
            "FRED_API_KEY",
            value="",
            placeholder=t('当前: {v1}', v1=fred_display)fred_display else t("未配置"),
        ...
        new_fmp_key = st.text_input(
            "FMP_API_KEY",
            value="",
            placeholder=t('当前: {v1}', v1=fmp_display)fmp_display else t("未配置"),
    ...
    with st.expander(t("🧠 LLM 智能助手配置"), expanded=False):
        try:
            ...
            if llm_status["enabled"]:
                st.success(t('✅ LLM 已启用 | 默认模型: {v1} | 可用: {v2}', v1=llm_status['default_model'], v2=', '.join(llm_status['available_models']))            elif llm_status["total_models"] > 0:
                st.warning(t('⚠️ 已配置 {v1} 个模型但无可用项 (Key 可能未填)。 模型: {v2}', v1=llm_status['total_models'], v2=', '.join(llm_status['all_models']))            else:
            ...
            if current_entry and current_entry.is_available():
                key_placeholder = t('当前: ***{v1} (留空不修改)', v1=current_entry.api_key[-4:])            else:
    ...
    with col2:
        if st.button(t("💾 保存配置到 .env"), type="primary", use_container_width=True):
            if not env_updates:
                ...
                try:
                    ...
                    if any(k.startswith("LLM_") for k in env_updates):
                        try:
                            ...
                            st.warning(t('LLM 配置重载失败: {v1}', v1=llm_e)                    st.info(t("💡 部分配置可能需要重启服务后生效"))
                except Exception as e:
                    st.error(t('❌ 保存失败: {v1}', v1=e)
    # 显示变更预览
    if env_updates:
        with st.expander(t('📝 待保存的变更 ({v1} 项)', v1=len(env_updates)):
            for key, value in env_updates.items():
...
def _ensure_chat_server():
    ...
    try:
        ...
        st.warning(t('智能助手服务启动失败: {v1}', v1=e)        return False
