...
PERIOD_CONFIG = {
    "1分": ("1m", False, t("日内1分钟K线 (最近7天)")),
    "5分": ("5m", False, t("日内5分钟K线 (最近60天)")),
    "15分": ("15m", False, t("日内15分钟K线 (最近60天)")),
    "30分": ("30m", False, t("日内30分钟K线 (最近60天)")),
    "60分": ("60m", False, t("日内60分钟K线 (最近60天)")),
    "日K": ("1d", True, t("日线K线 (前复权)")),
    "周K": ("1wk", True, t("周线K线")),
    "月K": ("1mo", True, t("月线K线")),
}
...
def _fetch_from_akshare(symbol: str, period: str, days_back: int = None) -> pd.DataFrame:
    ...
    try:
        ...
        st.warning(t('AKShare 数据获取失败: {v1}', v1=e)        return pd.DataFrame()
...
def _fetch_from_yfinance(symbol: str, interval: str, days_back: int = None) -> pd.DataFrame:
    ...
    try:
        ...
        st.warning(t('yfinance 数据获取失败: {v1}', v1=e)        return pd.DataFrame()
...
def render_kline_chart(
    ...
    if df is None or len(df) == 0:
        fig = go.Figure()
        fig.add_annotation(text=t("暂无数据"), showarrow=False, font_size=20)
        return fig
    ...
    # 创建子图
    subplot_titles = [t('📊 {v1} · {v2} K线', v1=symbol, v2=period)    if show_volume:
        subplot_titles.append(t("成交量"))
    if show_macd:
...
def render_stock_info(symbol: str, quote: dict, profile: dict, financials: dict):
    ...
    if quote:
        ...
        col1.metric(t("💰 最新价"), f"${price:.2f}", f"{change:+.2f}")
        col2.metric(t("📊 涨跌幅"), f"{change_pct:+.2f}%")
        col3.metric(t("🔵 开盘"), f"${quote.get('open', 0):.2f}")
        col4.metric(t("⬆️ 最高"), f"${quote.get('high', 0):.2f}")
        col5.metric(t("⬇️ 最低"), f"${quote.get('low', 0):.2f}")

        col6, col7, col8, col9, col10 = st.columns(5)
        col6.metric(t("📋 昨收"), f"${quote.get('prev_close', 0):.2f}")
        col7.metric(t("📈 成交量"), f"{quote.get('volume', 0):,}")
        col8.metric(t("📡 数据源"), quote.get("source", "-"))
        col9.metric(t("🕐 时间"), quote.get("timestamp", "-")[:19] if quote.get("timestamp") else "-")
    ...
    if profile:
        ...
        with col_l:
            ...
            if profile.get('description'):
                ...
                if len(desc) > 200:
                    st.caption(desc[:200] + "...")
                    with st.expander(t("📖 查看完整公司简介"), expanded=False):
                        st.markdown(desc)
                else:
                    st.caption(desc)
            ...
            if profile.get('finnhubIndustry'):
                info_parts.append(t('🏷️ 行业: {v1}', v1=profile['finnhubIndustry'])            if profile.get('country'):
                info_parts.append(t('🌍 国家: {v1}', v1=profile['country'])            if profile.get('exchange'):
                info_parts.append(t('🏛️ 交易所: {v1}', v1=profile['exchange'])            if profile.get('weburl'):
                info_parts.append(t('🔗 [官网]({v1})', v1=profile['weburl'])            if profile.get('employeeTotal'):
                info_parts.append(t('👥 员工: {v1:,}', v1=profile['employeeTotal'])            if info_parts:
        ...
        with col_r:
            if profile.get('marketCapitalization'):
                ...
                if mcap >= 1e12:
                    ...
                    mcap_str = f"${mcap:,.0f}"
                st.metric(t("🏦 市值"), mcap_str)
            if profile.get('shareOutstanding'):
                shares = profile['shareOutstanding']
                if shares >= 1e9:
                    st.metric(t("📊 流通股"), f"{shares/1e9:.2f}B")
                elif shares >= 1e6:
                    st.metric(t("📊 流通股"), f"{shares/1e6:.2f}M")
                else:
                    st.metric(t("📊 流通股"), f"{shares:,.0f}")
            if profile.get('ipo'):
                st.metric(t("📅 IPO日期"), profile['ipo'])
    else:
        # 无公司概况数据时显示提示
        st.info(t('💡 暂无 {v1} 的公司概况数据 (需配置 Finnhub API Key)', v1=symbol)
    # ── 基本面指标 ──
    if financials and financials.get('metric'):
        metric = financials['metric']
        st.markdown(t("#### 📋 关键财务指标"))
        ...
        col4.metric(t("营收增长"), f"{metric.get('revenueGrowth5Y', 'N/A')}")
        col5.metric(t("毛利率"), f"{metric.get('grossMarginTTM', 'N/A')}")
...
def page_chart_analysis():
    """行情分析页面 - 专业K线图"""
    st.title("📈 行情分析")
    st.caption(t("全周期K线图 · 技术指标 · 实时行情 · 基本面数据"))
    ...
    with col_ctrl:
        ...
        manual_symbol = st.text_input(
            t("🔍 输入股票代码"),
            value="",
            placeholder=t("如: AAPL, TSLA, NVDA"),
        ...
        # 布林带
        show_boll = st.checkbox(t("布林带 BOLL"), value=False, key="kline_boll")

        # 副图指标
        show_volume = st.checkbox(t("成交量"), value=True, key="kline_volume")
        ...
        # 刷新按钮
        if st.button(t("🔄 刷新数据"), key="kline_refresh", width='stretch'):
            st.cache_data.clear()
            st.rerun()
    ...
    with col_chart:
        if not selected_symbol:
            st.warning(t("请选择或输入股票代码"))
            return

        # 获取实时报价和公司信息
        with st.spinner(t("获取实时行情...")):
        ...
        with col_watch:
            ...
            if is_watched:
                if st.button(t("💔 取消关注"), key=f"btn_unwatch_chart_{selected_symbol}", use_container_width=True):
                    user_wl.remove(selected_symbol)
                    _save_user_watchlist(user_wl)
                    st.success(t('已取消关注 {v1}', v1=selected_symbol)                    st.rerun()
            else:
                if st.button(t("⭐ 关注该股票"), key=f"btn_watch_chart_{selected_symbol}", use_container_width=True):
                    if selected_symbol not in user_wl:
                        user_wl.append(selected_symbol)
                        _save_user_watchlist(user_wl)
                    st.success(t('已关注 {v1}', v1=selected_symbol)                    st.rerun()
            # 显示关注列表数量
            st.caption(t('📋 已关注 {v1} 只股票', v1=len(user_wl))
        with col_strat:
            # 为当前股票生成交易策略
            with st.expander(t("📈 为该股票生成交易策略"), expanded=False):
                default_name = t('行情策略_{v1}_{v2}', v1=selected_symbol, v2=datetime.now().strftime('%Y%m%d'))                strategy_name = st.text_input(
                    t("策略名称"), value=default_name, key=f"chart_strat_name_{selected_symbol}"
                )
                ...
                with col_s2:
                    max_position = st.slider(
                        t("最大仓位"), min_value=0.05, max_value=0.50, value=0.30, step=0.05,
                        format="%.0f%%", key=f"chart_strat_pos_{selected_symbol}",
                    )

                # 可选: 使用策略模板
                use_template = st.checkbox(
                    t("使用策略模板 (自动填充因子和参数)"), value=False,
                    ...
                    try:
                        ...
                        if templates:
                            ...
                            selected_template_id = st.selectbox(
                                t("选择模板"), options=list(tmpl_options.keys()),
                ...
                if st.button(t("🚀 创建策略"), type="primary",
                    ...
                    try:
                        ...
                        st.success(t("✅ 策略 '{v1}' 创建成功! (ID: {v2})", v1=strategy.name, v2=strategy_id)                        st.info(t('💡 股票池: {v1} | 模型: {v2} | 仓位: {v3:.0%}', v1=selected_symbol, v2=strategy.model_type, v3=max_position)                        st.caption(t("👉 前往「📈 策略管理」查看详情和回测"))
                    except Exception as e:
        ...
        with st.spinner(t('加载 {v1} {v2} K线数据...', v1=selected_symbol, v2=period)            df = fetch_kline_data(selected_symbol, period, days_back)

        if df is None or len(df) == 0:
            st.error(t('❌ 无法获取 {v1} 的K线数据，请检查股票代码或网络连接', v1=selected_symbol)            st.info(t("💡 提示: 日内分时数据需要安装 yfinance (`pip install yfinance`)"))
            return
        ...
        col_s1.metric(t("📊 K线条数"), f"{len(df)}")
        col_s2.metric(t("📅 起始"), df["datetime"].iloc[0].strftime("%Y-%m-%d %H:%M") if len(df) > 0 else "-")
        col_s3.metric(t("📅 结束"), df["datetime"].iloc[-1].strftime("%Y-%m-%d %H:%M") if len(df) > 0 else "-")
        if len(df) > 0:
            period_return = (df["close"].iloc[-1] / df["close"].iloc[0] - 1) * 100
            col_s4.metric(t("📈 区间收益"), f"{period_return:+.2f}%")
        ...
        # ─── K线数据表格 (可折叠) ───
        with st.expander(t('📋 {v1} {v2} 原始数据 (最近20条)', v1=selected_symbol, v2=period):
            display_df = df.tail(20).copy()
            display_df["datetime"] = display_df["datetime"].dt.strftime("%Y-%m-%d %H:%M")
            display_df = display_df[["datetime", "open", "high", "low", "close", "volume"]]
            display_df.columns = [t("时间"), t("开盘"), t("最高"), t("最低"), t("收盘"), t("成交量")]
