...
from src.ui.i18n import t, get_language
...
def get_account_info():
    ...
    try:
        ...
        # 记录具体原因，方便排查
        if data is None:
            st.session_state["_account_error"] = t("云端API返回空数据 (检查API地址配置)")
        elif not data.get("account"):
            err = data.get("positions", {})
            if isinstance(err, dict) and "error" in err:
                st.session_state["_account_error"] = t('账户获取失败: {v1}', v1=err['error'])
            else:
                st.session_state["_account_error"] = t("账户数据为空")
...
def render_sidebar():
    ...
    mode_text = t("📝 模拟盘") if is_paper else t("🔴 实盘")
    ...
    # ── 数据源切换 ──
    with st.sidebar.expander(t("🔌 数据源切换"), expanded=False):
        current_mode = st.session_state.get("data_source_mode", "local")
        mode_options = [t("本地直连"), t("云端API")]
        ...
        selected_mode = st.radio(
            ...
            format_func=t,  # i18n: 显示层翻译, 逻辑值保持中文键
        )

        new_mode = "cloud" if selected_mode == t("云端API") else "local"
        ...
        if new_mode == "cloud":
            ...
            if api_base:
                st.caption(t('📡 连接: `{v1}`', v1=api_base))
                # 快速健康检查
                try:
                    cc = _get_cloud_client()
                    ...
                    if health and health.get("status") == "ok":
                        ...
                        st.caption(f"⚠️ {t(health.get('status', '未知'))}")
                except Exception:
                    st.caption(t("⚠️ 无法连接云端"))
            else:
                st.caption(t("⚠️ 请输入云端 API 地址"))
        else:
            st.caption(t("💻 本地直连模式"))
    ...
    alpaca_text = t("已连接") if alpaca_on else t("未配置")
    finnhub_text = t("已连接") if finnhub_on else t("未配置")

    st.sidebar.markdown(t('\n    <div class="info-card">\n        <div class="info-card-title">📡 数据源</div>\n        <div class="info-row">\n            <span><span class="source-dot {v1}"></span>Alpaca</span>\n            <span class="value {v2}">{v3}</span>\n        </div>\n        <div class="info-row">\n            <span><span class="source-dot {v4}"></span>Finnhub</span>\n            <span class="value {v5}">{v6}</span>\n        </div>\n    </div>\n    ', v1=alpaca_dot, v2='value-green' if alpaca_on else 'value-red', v3=alpaca_text, v4=finnhub_dot, v5='value-green' if finnhub_on else 'value-red', v6=finnhub_text), unsafe_allow_html=True)

    # ── 风控参数 ──
    st.sidebar.markdown(t('\n    <div class="info-card">\n        <div class="info-card-title">🛡️ 风控</div>\n        <div class="info-row">\n            <span>单票仓位</span>\n            <span class="value value-blue">{v1:.0%}</span>\n        </div>\n        <div class="info-row">\n            <span>日最大亏损</span>\n            <span class="value value-red">{v2:.0%}</span>\n        </div>\n        <div class="info-row">\n            <span>最大回撤</span>\n            <span class="value value-red">{v3:.0%}</span>\n        </div>\n    </div>\n    ', v1=config.MAX_POSITION_PCT, v2=config.MAX_DAILY_LOSS, v3=config.MAX_DRAWDOWN), unsafe_allow_html=True)

    # ── 选股参数 ──
    st.sidebar.markdown(t('\n    <div class="info-card">\n        <div class="info-card-title">🎯 选股</div>\n        <div class="info-row">\n            <span>候选上限</span>\n            <span class="value">{v1}</span>\n        </div>\n        <div class="info-row">\n            <span>信号 Top-K</span>\n            <span class="value">{v2}</span>\n        </div>\n    </div>\n    ', v1=config.MAX_CANDIDATES, v2=config.TOP_K_SIGNALS), unsafe_allow_html=True)

    # ── 页面导航 ──
    st.sidebar.markdown("<div style='margin-top: 8px;'></div>", unsafe_allow_html=True)
    # i18n: 语言选择器 (用户手动切换, auto 模式自动检测浏览器/系统语言)
    from src.ui.i18n import render_language_selector
    ...
    page = st.sidebar.radio(
        ...
        index=0,
        label_visibility="collapsed",
        format_func=t,  # i18n: 显示层翻译, 内部逻辑保持中文键
    )
    return page
    ...
    try:
        ...
        with ctrl_col1:
            if not status["running"]:
                if st.button(t("▶️ 启动实时行情"), type="primary", key="btn_start_realtime"):
                    _start_realtime_stream()
                    st.rerun()
            else:
                if st.button(t("⏹️ 停止实时行情"), type="secondary", key="btn_stop_realtime"):
                    _stop_realtime_stream()
                    st.rerun()

        with ctrl_col2:
            st.metric(t("实时连接"), t("🟢 运行中") if status["running"] else t("⚪ 未启动"))
        with ctrl_col3:
            st.metric(t("缓存价格"), t('{v1} 只', v1=status['cached_prices']))
        with ctrl_col4:
            st.metric(t("告警数"), t('{v1} 条', v1=status['alerts_count']))
        ...
        src_col3.caption(t('🔧 模式: `{v1}`', v1=status['data_source']))
        ...
        if prices:
            ...
            for sym, quote in prices.items():
                ...
                price_rows.append({
                    t("代码"): sym,
                    t("价格"): f"",
                    t("涨跌"): f"{pnl_color} " if quote.change else "—",
                    t("涨跌幅"): f"{quote.change_pct:+.2f}%" if quote.change_pct else "—",
                    t("来源"): quote.source,
                    t("时间"): quote.timestamp.strftime("%H:%M:%S"),
                })
            st.dataframe(pd.DataFrame(price_rows), width='stretch', hide_index=True)
        else:
            st.info(t("💡 暂无实时价格数据。点击「启动实时行情」开始接收盘中数据。"))

        # 最近告警
        alerts = rm.get_alerts(limit=5)
        if alerts:
            st.markdown(t("**🚨 最近价格异动告警:**"))
            for a in reversed(alerts):
                st.text(f"  ⚠️ [{a.triggered_at.strftime('%H:%M:%S')}] {a.message}")

    except Exception as e:
        st.info(t('💡 实时行情模块加载中... ({v1})', v1=e))
...
def _start_realtime_stream():
    ...
    st.success(t('⚡ 实时行情已启动! 订阅 {v1} 只股票', v1=len(symbols)))
...
def page_overview():
    """总览首页"""
    st.title(t("📊 总览首页"))
    st.caption(t("账户资产、持仓分布、今日盈亏、告警状态"))

    # 账户概览卡片
    account = get_account_info()
    if account:
        col1, col2, col3, col4 = st.columns(4)
        col1.metric(t("💼 总资产"), f"${account['equity']:,.2f}")
        col2.metric(t("💵 现金"), f"${account['cash']:,.2f}")
        col3.metric(t("💪 贴买力"), f"${account['buying_power']:,.2f}")
        col4.metric(t("📋 交易模式"), t(account["trading_mode"]) if account["trading_mode"] in ("模拟盘", "实盘") else account["trading_mode"])
    else:
        _err = st.session_state.get("_account_error", "")
        if _err:
            st.warning(t('⚠️ 账户数据获取失败: {v1}', v1=_err))
        else:
            st.info(t("💡 Alpaca 未配置或无法连接，显示模拟数据"))
        col1, col2, col3, col4 = st.columns(4)
        col1.metric(t("💼 总资产"), "$100,000.00")
        col2.metric(t("💵 现金"), "$100,000.00")
        col3.metric(t("💪 购买力"), "$200,000.00")
        col4.metric(t("📋 交易模式"), t("📝 模拟盘"))
    ...
    if notifications:
        icons = {"INFO": "ℹ️", "WARNING": "⚠️", "ERROR": "❌",
                 "CRITICAL": "🚨", "SUCCESS": "✅"}
        rows = []
        # 告警标题/内容翻译映射 (后端数据为中文, 显示层按当前语言转换)
        _alert_titles = {
            "止损触发": t("止损触发"),
            "组合风控告警": t("组合风控告警"),
            "价格异动": t("价格异动"),
        ...
        def _tr_alert_text(s):
            """翻译告警文本: 先整句匹配, 再逐片段替换"""
            if not s:
                return s
            if s in _alert_titles:
                return _alert_titles[s]
            if get_language() == "zh":
                return s
            for zh, en in _alert_fragments.items():
                s = s.replace(zh, en)
            return s
        ...
        for n in reversed(notifications):
            ...
            rows.append({
                t("级别"): f"{icons.get(level, 'ℹ️')} {level}",
                t("时间"): n.get("timestamp", ""),
                t("标题"): _tr_alert_text(n.get("title", "")),
                t("内容"): _tr_alert_text(n.get("message", "")),
            })
...
def _save_screen_cache(result):
    ...
    try:
        ...
        st.warning(t('选股结果缓存保存失败: {v1}', v1=e))
...
def page_stock_screen():
    """智能选股页面"""
    st.title(t("🎯 智能选股"))
    st.caption(t("用自然语言选股，AI 自动扫描全美股并多维度评分"))

    # ─── 自然语言输入区 ───
    st.subheader(t("💬 自然语言选股"))
    st.caption(t('试试输入: "帮我选5支10美元的潜力股" 或 "选3只20-50美元的科技股"'))

    nl_input = st.text_input(
        t("输入选股需求"),
        placeholder=t("例如: 帮我选5支10美元的潜力股"),
        key="nl_screen_input",
    )

    # ─── 参数微调区 ───
    with st.expander(t("⚙️ 手动调整参数 (可选)"), expanded=False, key="expander_manual_screen_params"):
        col1, col2, col3 = st.columns(3)
        with col1:
            screen_count = st.number_input(t("选股数量"), 1, 20, 5, key="screen_count")
            screen_price_min = st.number_input(t("最低价格 ($)"), 0.0, 10000.0, 0.0, step=1.0, key="screen_price_min")
        with col2:
            screen_price_max = st.number_input(t("最高价格 ($)"), 0.0, 10000.0, 10.0, step=1.0, key="screen_price_max", help=t("0 = 不限"))
            screen_sector = st.selectbox(
                t("行业偏好"),
                [t(""), t("科技"), t("金融"), t("能源"), t("医疗"), t("消费")],
                key="screen_sector",
            )
        with col3:
            screen_keywords = st.text_input(t("关键词"), t("潜力"), key="screen_keywords", help=t("如: 潜力/成长/低价"))
            st.caption(t("💡 参数会自动从自然语言中提取，也可手动调整"))

    # ─── 执行选股 ───
    if st.button(t("🔍 开始选股"), type="primary", key="btn_screen"):
        ...
        if nl_input.strip():
            ...
            try:
                ...
                if llm_intent.intent == "stock_screen":
                    ...
                    st.info(t('🧠 AI 解析结果: 数量={v1}, 价格={v2}~{v3}, 行业={v4}, 关键词={v5}', v1=params.get('count', 5), v2=params.get('price_min', 0), v3=t(params.get('price_max', '不限')), v4=t(params.get('sector', '不限')), v5=t(params.get('keywords', '无'))))
                else:
                    st.warning(t('未识别为选股意图 (识别为: {v1})，使用手动参数', v1=llm_intent.intent))
            except Exception as e:
                st.warning(t('AI 解析失败 ({v1})，使用手动参数', v1=e))
        ...
        # 执行选股
        with st.spinner(t("🔍 正在扫描全美股并评分... (可能需要 30-60 秒)")):
            try:
                from src.engine.stock_screener import stock_screener
                result = stock_screener.screen(params, query=nl_input or t("手动选股"))
                st.session_state["screen_result"] = result
                # 持久化选股结果到本地, 下次启动自动恢复
                _save_screen_cache(result)
            except Exception as e:
                st.error(t('选股失败: {v1}', v1=e))
                return
    ...
    if result:
        if not result.picks:
            st.warning(result.plain_summary or t("未找到符合条件的股票"))
            return

        st.divider()
        st.subheader(t('📊 选股结果 (扫描 {v1} 只 -> 入选 {v2} 只)', v1=result.total_scanned, v2=len(result.picks)))

        # 概览指标
        col1, col2, col3, col4 = st.columns(4)
        col1.metric(t("扫描总数"), str(result.total_scanned))
        col2.metric(t("价格筛选后"), str(result.total_filtered))
        col3.metric(t("入选数量"), str(len(result.picks)))
        avg_score = sum(p.composite_score for p in result.picks) / len(result.picks)
        col4.metric(t("平均综合分"), f"{avg_score:.1f}")

        # 候选股票表格
        st.subheader(t("📋 入选股票详情"))

        # ── 获取实时行情 + 财务指标 (与行情分析页面一致) ──
        with st.spinner(t("⏳ 正在加载实时行情与财务指标...")):
            try:
        ...
        for p in result.picks:
            ...
            picks_data.append({
                ...
                t("行业"): p.sector or "-",
                # ── 实时行情 (与行情分析一致) ──
                t("最新价 ($)"): _fmt_price(q.get("price"), fallback=_fmt_price(p.price)),
                t("涨跌幅"): _fmt_pct(q.get("change_pct")),
                t("开盘 ($)"): _fmt_price(q.get("open")),
                t("最高 ($)"): _fmt_price(q.get("high")),
                t("最低 ($)"): _fmt_price(q.get("low")),
                t("昨收 ($)"): _fmt_price(q.get("prev_close")),
                t("成交量"): _fmt_volume(q.get("volume")),
                ...
                t("营收增长"): _fmt_metric(metric.get("revenueGrowth5Y"), "{:.2f}%"),
                t("毛利率"): _fmt_metric(metric.get("grossMarginTTM"), "{:.2f}%"),
                "Beta": _fmt_metric(metric.get("beta")),
                # ── 原有评分列 ──
                t("综合评分"): p.composite_score,
                t("技术面"): p.tech_score,
                t("基本面"): p.fundamental_score,
                t("情绪面"): p.sentiment_score,
                t("动量"): p.momentum_score,
                t("AI 推荐理由"): p.llm_reason or "-",
            })
        df_picks = pd.DataFrame(picks_data)
        st.dataframe(df_picks, width='stretch', hide_index=True)

        # 综合评分柱状图
        st.subheader(t("📊 综合评分对比"))
        fig_bar = go.Figure()
        fig_bar.add_trace(go.Bar(
        ...
        if len(result.picks) > 0:
            ...
            categories = [t("技术面"), t("基本面"), t("情绪面"), t("动量")]
            ...
            with col_radar:
                ...
                fig_radar.update_layout(
                    ...
                    title=t('{v1} 四维评分雷达图', v1=pick.symbol),
                    height=350,
                )
                st.plotly_chart(fig_radar, width='stretch')

                if pick.llm_reason:
                    st.info(t('💡 AI 推荐理由: {v1}', v1=pick.llm_reason))

            # ─── 公司简介 ───
            with st.spinner(t('正在加载 {v1} 公司信息...', v1=pick.symbol)):
                try:
                    from src.data.collector import data_collector
                    profile = None
                    ...
                    if profile and profile.get("description"):
                        ...
                        with col_info1:
                            st.metric(t("行业"), profile.get("sector") or profile.get("finnhubIndustry") or "-")
                        with col_info2:
                        ...
                        with col_info3:
                            ...
                            try:
                                ...
                                st.metric(t("员工数"), f"{employees:,}" if employees else "-")
                            except (ValueError, TypeError):
                                st.metric(t("员工数"), str(employees) if employees else "-")
                        with col_info4:
                            st.metric(t("网站"), profile.get("website", "-") or "-")

                        # 公司简介正文
                        description = profile.get("description", "")
                        if description and len(description) > 20:
                            with st.expander(t("📖 查看公司简介详情"), expanded=True):
                                st.write(description)
                    else:
                        st.caption(t("📭 暂无该公司简介信息"))
                except Exception as e:
                    st.caption(t('📭 公司信息加载失败: {v1}', v1=e))
            ...
            with col_action:
                ...
                with col_watch1:
                    if is_watched:
                        if st.button(t("💔 取消关注"), key=f"btn_unwatch_{pick.symbol}", width='stretch'):
                            user_watchlist.remove(pick.symbol)
                            _save_user_watchlist(user_watchlist)
                            st.success(t('已取消关注 {v1}', v1=pick.symbol))
                            st.rerun()
                    else:
                        if st.button(t("⭐ 关注该股票"), key=f"btn_watch_{pick.symbol}", width='stretch'):
                            if pick.symbol not in user_watchlist:
                                user_watchlist.append(pick.symbol)
                                _save_user_watchlist(user_watchlist)
                            st.success(t('已关注 {v1}', v1=pick.symbol))
                            st.rerun()
                with col_watch2:
                    watch_count = len(user_watchlist)
                    st.metric(t("已关注股票数"), f"{watch_count}")

                # 展示当前关注列表
                if user_watchlist:
                    watch_chips = " ".join(f"`{s}`" for s in user_watchlist[:15])
                    if len(user_watchlist) > 15:
                        watch_chips += t(' ...等 {v1} 只', v1=len(user_watchlist))
                    st.caption(t('📋 关注列表: {v1}', v1=watch_chips))
                ...
                # 功能2: 创建交易策略
                # ════════════════════════════════════════
                st.markdown(t("#### 📈 创建交易策略"))

                with st.expander(t("为该股票创建策略"), expanded=False, key=f"expander_strategy_{pick.symbol}"):
                    # 策略名称 (默认值包含股票代码和日期)
                    default_name = t('选股策略_{v1}_{v2}', v1=pick.symbol, v2=datetime.now().strftime('%Y%m%d'))
                    ...
                    if use_template:
                        try:
                            ...
                            st.warning(t('模板加载失败: {v1}', v1=e))

                    if st.button(t("🚀 创建策略"), type="primary", key=f"btn_create_strat_{pick.symbol}", width='stretch'):
                        try:
                            ...
                            st.success(t("✅ 策略 '{v1}' 创建成功! (ID: {v2})", v1=strategy.name, v2=strategy_id))
                            st.info(t('💡 股票池: {v1} | 模型: {v2} | 仓位: {v3:.0%}', v1=pick.symbol, v2=strategy.model_type, v3=max_position))
                            st.caption(t("👉 前往「📈 策略管理」查看详情和回测"))
                        except Exception as e:
                            st.error(t('创建失败: {v1}', v1=e))

            # ════════════════════════════════════════
            # 功能3: K线图展示
            # ════════════════════════════════════════
            st.divider()
            with st.expander(t('📊 {v1} K线图', v1=pick.symbol), expanded=True, key=f"expander_kline_{pick.symbol}"):
                # K线参数选择
                ...
                range_map = {t("3月"): 90, t("半年"): 180, t("1年"): 365, t("全部"): None}
                days_back = range_map.get(kline_range, 180)

                with st.spinner(t('正在加载 {v1} K线数据...', v1=pick.symbol)):
                    try:
                        ...
                        if kline_df is not None and len(kline_df) > 0:
                            ...
                            # 显示最近几日K线数据摘要
                            st.caption(t('📅 数据范围: {v1} ~ {v2} | 共 {v3} 根K线', v1=kline_df['datetime'].iloc[0].strftime('%Y-%m-%d'), v2=kline_df['datetime'].iloc[-1].strftime('%Y-%m-%d'), v3=len(kline_df)))
                        else:
                            st.warning(t('暂无 {v1} 的K线数据，请检查数据源', v1=pick.symbol))
                    except Exception as e:
                        st.warning(t('K线图加载失败: {v1}', v1=e))
        ...
        overview_data = {
            t("项目"): [t("筛选条件"), t("扫描总数"), t("价格筛选后"), t("入选数量"), t("平均综合分")],
            t("详情"): [
                t('{v1}, 选 {v2} 只', v1=_cond, v2=_count),
        ...
        # 风险提示
        st.caption(t("⚠️ 以上分析仅供参考，不构成投资建议"))

        # 后续操作提示
        st.divider()
        st.subheader(t("💡 后续操作"))
        st.markdown(
            t("  • 在上方 **🎯 四维评分详情** 中查看K线图、关注股票、创建交易策略\n  • 前往 **📈 策略管理** 查看和管理已创建的策略\n  • 或在对话中输入: \"为 AAPL 生成交易策略\"\n  • ⚠️ 以上分析仅供参考，不构成投资建议")
        )
    else:
        ...
        if "screen_result" not in st.session_state:
            ...
            if cached:
                ...
                st.rerun()
        st.info(t("💡 输入选股需求或调整参数后，点击「开始选股」"))
...
def _tr_strategy_name(name: str) -> str:
    """i18n: 翻译运行时生成的策略名 (如 '自动策略_lightgbm' -> 'Auto Strategy_lightgbm')"""
    if not isinstance(name, str):
        return name
    # 模式: 自动策略_{model_type}
    m = re.match(r"^自动策略_(\w+)$", name)
    if m:
        return t("自动策略_{v1}", v1=m.group(1))
    return name
...
def page_strategies():
    """策略管理"""
    st.title(t("📈 策略管理"))
    st.caption(t("策略列表、回测结果、评级展示、模板创建、组合管理"))

    # ─── 从模板创建策略 ───
    st.subheader(t("📋 从模板创建策略"))
    st.caption(t("无需理解因子和模型，选择一个模板即可快速创建策略"))
    ...
    if templates:
        ...
        for i, tmpl in enumerate(templates):
            ...
            with col:
                # i18n: 模板名/描述/tags 渲染层翻译 (engine 数据层保持中文)
                st.markdown(f"**{t(tmpl['name'])}** {risk_icon}")
                desc = t(tmpl["description"])
                st.caption(desc[:80] + "..." if len(desc) > 80 else desc)
                st.text(t('模型: {v1} | 因子: {v2}个', v1=tmpl['model_type'], v2=tmpl['factors_count']))
                st.text(t('股票池: {v1}只 | 仓位: {v2:.0%}', v1=tmpl['stock_pool_size'], v2=tmpl['max_position']))
                tags_str = " ".join(f"#{t(x)}" for x in tmpl["tags"][:3])
                st.caption(tags_str)
                if st.button(t('创建'), key=f"btn_tmpl_{tmpl['template_id']}"):
                    try:
                        strategy = strategy_templates.create_from_template(tmpl["template_id"])
                        if strategy:
                            st.success(t("✅ 策略 '{v1}' 创建成功! (ID: {v2})", v1=strategy.name, v2=strategy.strategy_id))
                            st.rerun()
                    except Exception as e:
                        st.error(t('创建失败: {v1}', v1=e))
    ...
    if not strategies_raw:
        st.info(t("📭 暂无策略，请从上方模板创建，或运行工作流自动生成"))
        return

    # 云端模式返回 dict 列表，本地模式返回对象列表，统一处理
    strategies = strategies_raw

    st.subheader(t('📋 策略列表 ({v1} 个)', v1=len(strategies)))
    ...
    for s in strategies:
        if isinstance(s, dict):
            # 云端模式: 已经是 dict
            strat_rows.append({
                t("策略ID"): s.get("strategy_id", ""),
                t("名称"): _tr_strategy_name(s.get("name", "")),
                t("模型"): s.get("model_type", ""),
                t("状态"): s.get("status", ""),
                t("股票池"): ", ".join(s.get("stock_pool", [])) if s.get("stock_pool") else "-",
                t("因子数"): len(s.get("factors", [])) if s.get("factors") else 0,
                t("最大仓位"): f"{s.get('max_position', 0):.0%}" if s.get("max_position") else "-",
                t("调仓频率"): s.get("rebalance_freq", "-"),
                t("创建时间"): s.get("created_at", ""),
            })
        else:
            # 本地模式: StrategyConfig 对象
            strat_rows.append({
                t("策略ID"): s.strategy_id,
                t("名称"): _tr_strategy_name(s.name),
                t("模型"): s.model_type,
                t("状态"): s.status,
                t("股票池"): ", ".join(s.stock_pool) if s.stock_pool else "-",
                t("因子数"): len(s.factors),
                t("最大仓位"): f"{s.max_position:.0%}",
                t("调仓频率"): s.rebalance_freq,
                t("创建时间"): s.created_at.strftime("%Y-%m-%d %H:%M"),
            })
    ...
    # ─── 多策略组合管理 ───
    st.subheader(t("📊 多策略组合管理"))
    st.caption(t("创建多策略组合，支持等权/风险平价/绩效加权分配"))
    ...
    try:
        ...
        # 创建组合
        with st.expander(t("➕ 创建新组合"), expanded=False):
            # 策略多选
            ...
            with col_p1:
                portfolio_name = st.text_input(t("组合名称"), t("我的组合"), key="portfolio_name")
            with col_p2:
                method = st.selectbox(
                    t("权重分配方式"),
                    [t("等权分配"), t("风险平价"), t("绩效加权")],
                    key="portfolio_method",
                )
            with col_p3:
                risk_budget = st.number_input(
                    t("风险预算"), 0.01, 0.50, 0.10, 0.01,
                    key="portfolio_risk_budget",
                )

            if st.button(t("创建组合"), key="btn_create_portfolio"):
                if len(selected_sids) < 2:
                    st.warning(t("请至少选择 2 个策略"))
                else:
                    method_map = {
                        t("等权分配"): AllocationMethod.EQUAL,
                        t("风险平价"): AllocationMethod.RISK_PARITY,
                        t("绩效加权"): AllocationMethod.PERFORMANCE,
                    }
                    try:
                        ...
                        st.success(t("✅ 组合 '{v1}' 创建成功! (ID: {v2})", v1=p.name, v2=p.portfolio_id))
                        st.rerun()
                    except Exception as e:
                        st.error(t('创建失败: {v1}', v1=e))
        ...
        if portfolios:
            for p in portfolios:
                ...
                with st.expander(f"📊 {p.name} ({p.portfolio_id}) - {status.allocation_method}"):
                    ...
                    col_s1.metric(t("策略数"), str(status.strategy_count))
                    col_s2.metric(t("权重总和"), f"{status.total_weight:.0%}")
                    col_s3.metric(t("上次再平衡"), status.last_rebalance.strftime("%Y-%m-%d %H:%M") if status.last_rebalance else t("未执行"))
                    ...
                    if status.strategy_weights:
                        ...
                        fig_w = px.pie(
                            weight_df, values=t("权重"), names=t("策略ID"),
                            title=t('{v1} 策略权重分布', v1=p.name), hole=0.4,
                        )
                    ...
                    with col_r1:
                        if st.button(t("🔄 重新分配权重"), key=f"btn_realloc_{p.portfolio_id}"):
                            try:
                                new_weights = portfolio_manager.allocate_weights(p)
                                st.success(t('✅ 权重已更新: {v1}', v1=new_weights))
                                st.rerun()
                            except Exception as e:
                                st.error(t('分配失败: {v1}', v1=e))
                    with col_r2:
                        if st.button(t("🗑️ 删除组合"), key=f"btn_del_port_{p.portfolio_id}"):
            ...
            st.info(t("📭 暂无组合，请在上方创建"))

    except Exception as e:
        st.info(t('组合管理模块未就绪: {v1}', v1=e))
    ...
    selected = st.selectbox(t("选择策略"), strat_names)
    ...
    if selected:
        ...
        if selected_strat:
            ...
            with col1:
                st.metric(t("策略ID"), _sid)
                st.metric(t("模型类型"), _model)
                st.metric(t("状态"), _status)
            with col2:
                st.metric(t("股票池"), ", ".join(_pool) if _pool else "-")
                st.metric(t("因子数"), str(len(_factors)))

                # 获取回测结果 (通过 cloud_client)
                cc = _get_cloud_client()
                bt = cc.get_backtest(_sid)
                grade = bt.get("grade", t("未回测")) if bt else t("未回测")
                grade_icon = {"A": "🟢", "B": "🔵", "C": "🟡", "D": "🔴"}.get(grade, "⚪")
                st.metric(t("评级"), f"{grade_icon} {grade}")
            ...
            if bt:
                ...
                col1, col2, col3, col4 = st.columns(4)
                col1.metric(t("年化收益"), f"{bt.get('annual_return', 0):.1%}")
                col2.metric(t("夏普比率"), f"{bt.get('sharpe_ratio', 0):.2f}")
                col3.metric(t("最大回撤"), f"{bt.get('max_drawdown', 0):.1%}")
                col4.metric(t("胜率"), f"{bt.get('win_rate', 0):.1%}")
...
def page_trades():
    ...
    if pos_data and "positions" in pos_data and pos_data["positions"]:
        ...
        # 汇总
        col1, col2, col3 = st.columns(3)
        col1.metric(t("持仓数量"), str(pos_data.get("position_count", 0)))
        col2.metric(t("持仓市值"), f"${pos_data.get('total_market_value', 0):,.2f}")
        col3.metric(t("浮动盈亏"), f"${pos_data.get('total_unrealized_pnl', 0):,.2f}")
    else:
        st.info(t("📭 当前无持仓"))
    ...
    # 通知历史 (交易相关)
    st.subheader(t("📜 交易通知历史"))
    notifications = get_notifications(30)
    if notifications:
        # 过滤交易相关通知
        trade_keywords = [t("交易"), t("执行"), t("清仓"), t("止损"), t("买入"), t("卖出"), t("订单")]
        trade_notifs = [
...
def page_research():
    """市场调研"""
    st.title(t("🔍 市场调研"))
    st.caption(t("最新调研报告、候选股票、新闻情绪、关注股票深度调研"))
    ...
    # 区域1: 全市场调研 (带主题输入)
    # ════════════════════════════════════════
    st.subheader(t("🌐 全市场调研"))
    research_topic = st.text_input(
        t("关注主题 (可选)"),
        placeholder=t("如: AI芯片, 新能源, 半导体"),
        key="research_topic_input",
        help=t("输入行业/主题关键词，调研时优先匹配相关股票"),
    )
    if st.button(t("🔄 执行市场调研"), type="primary", key="btn_research_execute"):
        try:
            registry = _get_agent_registry()
            research_agent = registry.get("research")
            if research_agent:
                with st.spinner(t("正在调研市场...")):
                ...
                st.error(t("Research Agent 未就绪"))
        except Exception as e:
            st.error(t('调研失败: {v1}', v1=e))

    st.caption(t('候选上限: {v1}', v1=config.MAX_CANDIDATES))
    ...
    # 区域2: 单股/多股深度调研
    # ════════════════════════════════════════
    st.subheader(t("🔎 单股深度调研"))
    stock_input = st.text_input(
        t("输入股票代码"),
        placeholder=t("如: AAPL  或  AAPL,NVDA,TSLA  (逗号分隔多个)"),
        key="research_stock_input",
    )
    if st.button(t("🔍 调研指定股票"), type="secondary", key="btn_research_single"):
        if not stock_input.strip():
            st.warning(t("请输入股票代码"))
        else:
    ...
    # 区域3: 关注股票专属调研
    # ════════════════════════════════════════
    st.subheader(t("⭐ 我的关注股票调研"))
    watchlist = config.WATCHLIST if config.WATCHLIST else config.DEFAULT_WATCHLIST
    watchlist_display = ", ".join(watchlist)
    st.caption(t('📋 关注列表 ({v1} 只): {v2}', v1=len(watchlist), v2=watchlist_display))

    if st.button(t("⭐ 调研全部关注股票"), type="secondary", key="btn_research_watchlist"):
        try:
            registry = _get_agent_registry()
            research_agent = registry.get("research")
            if research_agent:
                with st.spinner(t('正在调研 {v1} 只关注股票... (可能需要30-60秒)', v1=len(watchlist))):
                    results = research_agent.research_watchlist()
                    st.session_state["research_watchlist_results"] = results
                st.success(t('✅ {v1} 只关注股票调研完成!', v1=len(results)))
            else:
                st.error(t("Research Agent 未就绪"))
        except Exception as e:
            st.error(t('关注股票调研失败: {v1}', v1=e))
    ...
    if single_results:
        st.subheader(t('🔎 单股调研结果 ({v1} 只)', v1=len(single_results)))
    ...
    if watchlist_results:
        st.subheader(t('⭐ 关注股票调研结果 ({v1} 只)', v1=len(watchlist_results)))
    ...
    if report:
        ...
        if hasattr(report, "market_sentiment"):
            ...
            if sentiment > 0.3:
                sentiment_label = t("🟢 偏乐观")
            elif sentiment > 0:
                sentiment_label = t("🟡 微乐观")
            elif sentiment > -0.3:
                sentiment_label = t("🟠 偏悲观")
            else:
                sentiment_label = t("🔴 悲观")

            col1, col2 = st.columns(2)
            col1.metric(t("市场情绪"), f"{sentiment_label} ({sentiment:+.2f})")
            col2.metric(t("候选股票数"), str(len(report.candidate_stocks)))
        ...
        if hasattr(report, "plain_summary") and report.plain_summary:
            ...
            for line in report.plain_summary.strip().split("\n"):
                ...
                if ":" in stripped:
                    ...
                    summary_rows.append({t("项目"): parts[0].strip(), t("内容"): parts[1].strip()})
                else:
                    summary_rows.append({t("项目"): "", t("内容"): stripped})
            if summary_rows:
    ...
    if not report and not single_results and not watchlist_results:
        st.info(t("💡 点击上方按钮执行市场调研、单股调研或关注股票调研"))
...
def _render_single_stock_research(r: dict):
    ...
    with st.expander(
        t('{v1} - {v2} | {v3} 情绪: {v4} ({v5:+.2f})', v1=r['symbol'], v2=r['name'], v3=sent_icon, v4=t(r.get('sentiment_label', '中性')), v5=sentiment),
        expanded=True,
    ):
        col1, col2, col3, col4 = st.columns(4)
        col1.metric(t("代码"), r["symbol"])
        col2.metric(t("名称"), r["name"])
        col3.metric(t("行业"), r.get("sector") or "-")
        col4.metric(t("情绪分"), f"{sentiment:+.2f}")
        ...
        if profile:
            with st.expander(t("🏢 公司详情"), expanded=False):
                col_p1, col_p2, col_p3 = st.columns(3)
                with col_p1:
                    st.metric(t("市值"), f"${profile.get('marketCapitalization', 0):,.0f}" if profile.get('marketCapitalization') else "-")
                with col_p2:
                    st.metric("PE", f"{profile.get('pe', 0):.1f}" if profile.get('pe') else "-")
                with col_p3:
                    st.metric(t("股息率"), f"{profile.get('dividend', 0):.2f}%" if profile.get('dividend') else "-")
...
def _render_watchlist_research(results: list[dict]):
    ...
    for r in results:
        ...
        rows.append({
            t("代码"): r["symbol"],
            t("名称"): r.get("name", "-"),
            t("行业"): r.get("sector") or "-",
            t("情绪"): f"{sent_icon} {sentiment:+.2f}",
            t("新闻数"): news_count,
            t("推荐理由"): r.get("plain_reason", "-")[:80],
        })
    st.dataframe(pd.DataFrame(rows), width='stretch', hide_index=True)

    # 可展开详情
    st.markdown(t("**📋 点击查看个股详情:**"))
    for r in results:
        _render_single_stock_research(r)
...
def page_macro():
    """宏观经济指标页面"""
    st.title(t("🌐 宏观经济指标"))
    st.caption(t("美联储宏观数据 (FRED) · 美债收益率曲线 · 宏观风险评分"))
    ...
    if not _is_cloud_mode():
        if not dc.fred.available:
            st.warning(t("⚠️ FRED API 未配置，无法获取宏观数据。请在 .env 中设置 FRED_API_KEY"))
            return

    # ─── 宏观风险评分卡片 ───
    st.subheader(t("📊 宏观风险评分"))
    with st.spinner(t("正在获取宏观数据...")):
        macro_data = get_macro_dashboard_data()

    if not macro_data:
        st.error(t("获取宏观数据失败，请检查 FRED API 配置"))
        return
    ...
    if risk_score:
        ...
        level = t(risk_score.get("level", "未知"))
        ...
        with col_r1:
            st.metric(t("风险评分"), f"{score}/{max_score}")
        with col_r2:
            # 评分进度条
            fig_gauge = go.Figure(go.Indicator(
                ...
                title={"text": t('风险等级: {v1}', v1=level)},
            ))
            fig_gauge.update_layout(height=250)
            st.plotly_chart(fig_gauge, width='stretch')
        with col_r3:
            st.metric(t("风险等级"), level)

        st.info(t('💡 建议: {v1}', v1=t(recommendation)))
    ...
    for category, label in category_names.items():
        ...
        with st.expander(t('{v1} ({v2} 个指标)', v1=label, v2=len(items)), expanded=True):
            rows = []
            for item in items:
                ...
                rows.append({
                    t("指标"): t(item.get("name", "")),
                    t("最新值"): val_str,
                    t("单位"): t(unit),
                    t("环比"): mom_str,
                    t("同比"): yoy_str,
                    t("更新日期"): item.get("date", ""),
                    t("频率"): t(item.get("frequency", "")),
                })
    ...
    if yield_curve:
        ...
        # 倒挂警告
        if is_inverted:
            st.error(t('🚨 {v1} - 收益率倒挂是经济衰退的领先指标!', v1=t(inversion_signal)))
        else:
            st.success(f"✅ {t(inversion_signal)}")

        col_y1, col_y2 = st.columns(2)
        with col_y1:
            st.metric(t("10Y-2Y 利差"), f"{t10y2y:.3f}%" if t10y2y is not None else "N/A")
        with col_y2:
            st.metric(t("10Y-3M 利差"), f"{t10y3m:.3f}%" if t10y3m is not None else "N/A")

        # 收益率曲线图
        if curve_data:
            fig_curve = go.Figure()
            fig_curve.add_trace(go.Scatter(
                x=[t(d["tenor"]) for d in curve_data],
    ...
    # ─── VIX 恐慌指数 ───
    st.subheader(t("😱 VIX 恐慌指数"))
    try:
        cc = _get_cloud_client()
        vix_data = cc.get_vix()
        if vix_data:
            col_v1, col_v2, col_v3, col_v4 = st.columns(4)
            col_v1.metric(t("VIX 值"), f"{vix_data['value']:.2f}")
            col_v2.metric(t("涨跌"), f"{vix_data['change']:+.2f}")
            col_v3.metric(t("涨跌幅"), f"{vix_data['change_pct']:+.2f}%")
            col_v4.metric(t("风险等级"), t(vix_data["risk_level"]))

            color_map = {"green": "🟢", "lightgreen": "🟢", "yellow": "🟡", "orange": "🟠", "red": "🔴"}
            st.info(t('{v1} VIX 风险等级: {v2} | 数据时间: {v3}', v1=color_map.get(vix_data['risk_color'], '⚪'), v2=t(vix_data['risk_level']), v3=vix_data.get('timestamp', 'N/A')))
        else:
            st.info(t("获取 VIX 数据失败"))
    except Exception as e:
        st.warning(t('VIX 数据获取失败: {v1}', v1=e))

    st.divider()

    # ─── 数据源信息 ───
    st.subheader(t("📡 数据源信息"))
    st.text(t("数据来源: FRED (Federal Reserve Economic Data)"))
    config = _get_config()
    fred_key_display = f"***{config.FRED_API_KEY[-4:]}" if config.has_fred_key() else t("未配置")
    st.text(f"API Key: {fred_key_display}")
    st.text(t("缓存策略: 宏观数据6小时, 收益率曲线12小时, VIX 5分钟"))
    st.caption(t("⚠️ 宏观数据有发布延迟，CPI/就业等月度指标通常滞后1-2个月"))
...
def page_options():
    """期权数据页面"""
    st.title(t("📊 期权数据"))
    st.caption(t("期权链 · PCR比率 · 隐含波动率 · 到期日选择 (Yahoo Finance)"))
    ...
    with col_s1:
        symbol_input = st.text_input(
            ...
            help=t("输入美股代码，如 AAPL, TSLA, NVDA")
        ).upper().strip()
    ...
    with st.spinner(t('正在获取 {v1} 期权链数据...', v1=symbol_input)):
        # 先获取一次以拿到到期日列表
        opt_data = get_options_chain_data(symbol_input, "")

    if not opt_data:
        st.error(t('获取 {v1} 期权链失败，可能该股票没有期权数据', v1=symbol_input))
        return
    ...
    if selected_exp and selected_exp != opt_data.get("selected_expiration"):
        with st.spinner(t('正在获取 {v1} 到期日期权数据...', v1=selected_exp)):
    ...
    col_o1.metric(t("标的现价"), f"")
    col_o2.metric(t("PCR (成交量)"), f"{pcr_volume:.3f}", help=t("Put-Call Ratio < 1 偏多, > 1 偏空"))
    col_o3.metric(t("PCR (持仓量)"), f"{pcr_oi:.3f}", help=t("基于未平仓合约量的 PCR"))
    col_o4.metric(t("ATM 隐含波动率"), f"{atm_iv:.2f}%", help=t("平值期权隐含波动率"))
    col_o5.metric(t("到期日"), selected_exp)
    ...
    st.info(t('📊 PCR (成交量) 解读: {v1} | 看涨 {v2:,} vs 看跌 {v3:,}', v1=pcr_label, v2=summary.get('call_volume', 0), v3=summary.get('put_volume', 0)))
    ...
    if isinstance(calls_df, pd.DataFrame) and not calls_df.empty:
        # 选择关键列
        call_cols_map = {
            "strike": t("行权价"), "lastPrice": t("最新价"), "bid": t("买价"), "ask": t("卖价"),
            "volume": t("成交量"), "openInterest": t("未平仓"), "impliedVolatility": t("隐含波动率(%)"),
            "change": t("涨跌"), "percentChange": t("涨跌幅(%)"), "inTheMoney": t("实值"),
        }
        ...
        if available_call_cols:
            ...
            st.subheader(t('📈 看涨期权 - {v1} ({v2} 个合约)', v1=selected_exp, v2=len(display_calls)))
            ...
            if "volume" in calls_df.columns and "strike" in calls_df.columns:
                ...
                if not top_calls.empty:
                    ...
                    fig_call_vol.update_layout(
                        title=t("看涨期权 Top 20 成交量 & 未平仓 (按行权价)"),
                        xaxis_title=t("行权价"),
                        yaxis_title=t("数量"),
                        barmode="group",
                        height=350,
    ...
    if isinstance(puts_df, pd.DataFrame) and not puts_df.empty:
        ...
        if available_put_cols:
            ...
            st.subheader(t('📉 看跌期权 - {v1} ({v2} 个合约)', v1=selected_exp, v2=len(display_puts)))
            ...
            if "volume" in puts_df.columns and "strike" in puts_df.columns:
                ...
                if not top_puts.empty:
                    ...
                    fig_put_vol.update_layout(
                        title=t("看跌期权 Top 20 成交量 & 未平仓 (按行权价)"),
                        xaxis_title=t("行权价"),
                        yaxis_title=t("数量"),
                        barmode="group",
                        height=350,
        ...
        st.warning(t("无看跌期权数据"))

    st.divider()

    # ─── 隐含波动率微笑曲线 ───
    st.subheader(t("😊 隐含波动率微笑曲线"))
    if (isinstance(calls_df, pd.DataFrame) and not calls_df.empty and
        ...
        if underlying_price > 0:
            fig_smile.add_vline(
                ...
                line_dash="dash",
                line_color="blue",
                annotation_text=t('现价 '),
            )

        fig_smile.update_layout(
            title=t('{v1} 隐含波动率曲线 - {v2}', v1=symbol_input, v2=selected_exp),
            xaxis_title=t("行权价"),
            yaxis_title=t("隐含波动率 (%)"),
            height=400,
        )
        ...
        st.info(t("隐含波动率数据不完整"))

    st.divider()

    # ─── 期权希腊字母说明 ───
    with st.expander(t("📖 期权数据解读指南"), expanded=False):
...
def page_crypto():
    """加密货币交易面板"""
    st.title(t("🪙 加密货币"))
    st.caption(t("加密货币持仓、风控状态、价格看板 (24/7 交易)"))
    ...
    # ── 加密货币配置概览 ──
    col1, col2, col3, col4 = st.columns(4)
    col1.metric(t("🔄 数据源"), config.CRYPTO_DATA_SOURCE.upper())
    col2.metric(t("📊 交易对数"), len(config.get_crypto_pairs()))
    col3.metric(t("🛡️ 单币仓位上限"), f"{config.CRYPTO_MAX_POSITION_PCT:.0%}")
    col4.metric(t("🚫 止损线"), f"-{config.CRYPTO_SINGLE_STOP_LOSS:.0%}")
    ...
    try:
        ...
        if alpaca_client.available:
            ...
            if account:
                ...
                col2.metric(t("💵 现金"), f"")
                col3.metric(t("💪 购买力"), f"")
                mode_label = t("📝 模拟盘") if account.trading_mode.value == "paper" else t("🔴 实盘")
                col4.metric(t("📋 交易模式"), mode_label)
            else:
                st.info(t("💡 无法获取账户信息"))
        else:
            st.info(t("💡 Alpaca 未配置, 无法获取账户数据"))
    except Exception as e:
        st.warning(t('获取账户信息异常: {v1}', v1=e))

    st.divider()

    # ── 加密货币持仓 ──
    st.subheader(t("📈 加密货币持仓"))
    try:
        cc = _get_cloud_client()
        ...
        if account_data:
            ...
            col2.metric(t("💵 现金"), f"")
            col3.metric(t("💪 购买力"), f"")
            mode_label = t(account_data.get("trading_mode", "📝 模拟盘"))
            col4.metric(t("📋 交易模式"), mode_label)
    ...
    if price_results:
        ...
        col_info1.caption(t('✅ {v1} 个交易对可获取价格', v1=available_count))
        if unavailable_count > 0:
            col_info2.caption(t('❌ {v1} 个交易对不可用', v1=unavailable_count))
        ...
        styled = df_prices.style.format(
            {t("最新价格 ($)"): lambda v: f"${v:,.2f}" if v is not None else "N/A"},
            na_rep="N/A",
        )
        styled = styled.map(_color_change, subset=[t("24h涨跌幅")])
        ...
        st.info(t("💡 暂无可用价格数据，请检查 Alpaca API 配置"))

    st.divider()

    # ── 手动交易面板 ──
    st.subheader(t("🛒 手动交易"))
    st.caption(t("手动买入/卖出加密货币 (Paper Trading 模拟盘)"))

    try:
        from src.execution.alpaca_client import alpaca_client

        if not alpaca_client.available:
            st.info(t("💡 Alpaca 未配置，无法进行交易"))
        else:
            ...
            with col_trade1:
                ...
                selected_pair = st.selectbox(t("交易对"), trade_pairs, key="crypto_trade_pair")
            ...
            with col_trade3:
                ...
                for r in price_results:
                    if r["symbol"] == selected_pair and r["available"]:
                        ...
                        break
                if current_price:
                    st.metric(t("当前价格"), f"${current_price:,.2f}")
                    est_value = current_price * trade_qty
                    st.caption(t('预估金额: ${v1:,.2f}', v1=est_value))
                    # 最小订单金额校验提示
                    if est_value < 10.0:
                        st.warning(t('⚠️ 预估金额 ${v1:.2f} < $10, Alpaca 最小订单金额为 $10, 请增加数量', v1=est_value))
                else:
                    st.metric(t("当前价格"), "N/A")
            ...
            with col_buy:
                if st.button(t("📈 买入"), type="primary", key="btn_crypto_buy"):
                    if not selected_pair or trade_qty <= 0:
                        st.warning(t("请选择交易对并输入有效数量"))
                    else:
                        # 前置校验: 最小订单金额
                        if current_price and current_price * trade_qty < 10.0:
                            st.error(t('❌ 订单金额 ${v1:.2f} < $10 (Alpaca 最小限额), 请增加数量', v1=current_price * trade_qty))
                        else:
                            with st.spinner(t('正在买入 {v1} {v2}...', v1=trade_qty, v2=selected_pair)):
                                try:
                                    ...
                                    if order:
                                        st.success(t('✅ 买入订单已提交! ID: {v1}', v1=order.order_id))
                                        st.info(t('📊 {v1} × {v2:.4f} @ 市价', v1=selected_pair, v2=trade_qty))
                                    else:
                                        st.error(t("订单提交失败，请检查 API 配置"))
                                except Exception as e:
                                    st.error(t('买入失败: {v1}', v1=e))

            with col_sell:
                if st.button(t("📉 卖出"), type="secondary", key="btn_crypto_sell"):
                    if not selected_pair or trade_qty <= 0:
                        st.warning(t("请选择交易对并输入有效数量"))
                    else:
                        # 前置校验: 最小订单金额
                        if current_price and current_price * trade_qty < 10.0:
                            ...
                            with st.spinner(t('正在卖出 {v1} {v2}...', v1=trade_qty, v2=selected_pair)):
                                try:
                                    order = alpaca_client.submit_crypto_order(
                                        symbol=selected_pair,
                                        qty=trade_qty,
                                    ...
                                    if order:
                                        st.success(t('✅ 卖出订单已提交! ID: {v1}', v1=order.order_id))
                                        st.info(t('📊 {v1} × {v2:.4f} @ 市价', v1=selected_pair, v2=trade_qty))
                                    else:
                                        st.error(t("订单提交失败，请检查 API 配置"))
                                except Exception as e:
                                    st.error(t('卖出失败: {v1}', v1=e))

            with col_info:
                st.caption(t("💡 交易在 Alpaca Paper Trading 模拟盘执行，不涉及真实资金"))
                st.caption(t("⚠️ 加密货币 24/7 交易，订单使用 GTC (撤销前有效)"))

    except Exception as e:
        st.warning(t('手动交易面板加载异常: {v1}', v1=e))
        st.warning(t('获取价格数据异常: {v1}', v1=e))

    st.divider()

    # ── 风控状态 ──
    st.subheader(t("🛡️ 加密货币风控"))
    col1, col2, col3, col4 = st.columns(4)
    col1.metric(t("单币仓位上限"), f"{config.CRYPTO_MAX_POSITION_PCT:.0%}")
    col2.metric(t("日最大亏损"), f"{config.CRYPTO_MAX_DAILY_LOSS:.0%}")
    col3.metric(t("最大回撤"), f"{config.CRYPTO_MAX_DRAWDOWN:.0%}")
    col4.metric(t("日交易次数上限"), f"{config.CRYPTO_DAILY_TRADE_LIMIT}")

    col1, col2, col3 = st.columns(3)
    col1.metric(t("止损线"), f"-{config.CRYPTO_SINGLE_STOP_LOSS:.0%}")
    col2.metric(t("最大持仓数"), f"{config.CRYPTO_MAX_TOTAL_POSITIONS}")
    col3.metric(t("Paper 最少天数"), t('{v1} 天', v1=config.CRYPTO_MIN_PAPER_DAYS))
        ...
        if alpaca_client.available:
            ...
            if account:
                ...
                if risk_status.is_alert:
                    ...
                    st.success(t("✅ 风控正常, 无告警"))
    except Exception as e:
        st.info(t('风控检查不可用: {v1}', v1=e))

    st.divider()

    # ── 交易对列表 ──
    st.subheader(t("📋 交易对列表"))
    ...
    with col1:
        if st.button(t("🔄 运行加密货币交易循环"), help=t("执行: 调研→信号→风控→执行→监控")):
            with st.spinner(t("正在执行加密货币交易循环...")):
                try:
                    from src.workflow.engine import workflow_engine
                    result = workflow_engine.run_crypto_loop()
                    if result.get("status") == "success":
                        st.success(t("✅ 交易循环执行完成!"))
                    else:
                        st.warning(t('⚠️ 执行完成 (有异常): {v1}', v1=result.get('status')))
                    # 显示执行摘要
                    for step_name, step_status in result.get("steps", {}).items():
                        st.text(f"  {step_name}: {step_status}")
                except Exception as e:
                    st.error(t('执行失败: {v1}', v1=e))

    with col2:
        if st.button(t("📊 刷新加密货币快照"), help=t("获取最新的持仓和风控状态")):
            st.rerun()
...
def page_settings():
    """系统设置"""
    st.title(t("⚙️ 系统设置"))
    st.caption(t("配置查看、工作流手动触发、紧急止损、实盘迁移"))
    ...
    if st.button(t("💾 保存选股配置")):
        config.MAX_CANDIDATES = int(new_max_candidates)
        config.TOP_K_SIGNALS = int(new_top_k)
        st.success(t('✅ 已更新: 候选上限={v1}, Top-K={v2}', v1=config.MAX_CANDIDATES, v2=config.TOP_K_SIGNALS))

    st.divider()

    # 工作流手动触发
    st.subheader(t("🔄 工作流管理"))
    workflows = get_workflow_names()
    if workflows:
        col1, col2 = st.columns([2, 1])
        with col1:
            selected_wf = st.selectbox(t("选择工作流"), workflows)
        with col2:
            st.write("")  # 占位
            st.write("")  # 占位

        user_input = st.text_input(t("输入参数 (可选)"), "")

        if st.button(t("▶️ 运行工作流"), type="primary"):
            with st.spinner(t('正在运行 {v1}...', v1=selected_wf)):
                try:
                    ...
                    st.success(t('✅ 工作流完成: {v1}', v1=result.get('status', 'unknown')))

                    # 显示步骤结果
                    steps = result.get("steps", {})
                    if steps:
                        st.subheader(t("步骤执行结果"))
                        for step_name, step_result in steps.items():
                            if step_result is not None:
                                st.text(t('  ✅ {v1}: 已完成', v1=step_name))
                            else:
                                st.text(t('  ❌ {v1}: 失败或跳过', v1=step_name))
                except Exception as e:
                    st.error(t('工作流执行失败: {v1}', v1=e))
    else:
        st.info(t("工作流引擎未初始化"))
    ...
    # 紧急止损
    st.subheader(t("🚨 紧急操作"))
    st.warning(t("⚠️ 以下操作不可撤销，请谨慎执行!"))

    col1, col2 = st.columns(2)
    with col1:
        if st.button(t("🚨 紧急清仓"), type="secondary"):
            st.session_state["confirm_emergency"] = True

    with col2:
        if st.button(t("📊 运行监控快照")):
            with st.spinner(t("正在检查持仓...")):
                try:
                    registry = _get_agent_registry()
                    ...
                    if monitor:
                        ...
                        if isinstance(result, dict):
                            ...
                            if alerts:
                                st.warning(t('检测到 {v1} 条告警:', v1=len(alerts)))
                                for a in alerts:
                                    st.text(f"  ⚠️ {a}")
                            else:
                                st.info(t("无告警，组合运行正常"))
                    else:
                        st.error(t("Monitor Agent 未就绪"))
                except Exception as e:
                    st.error(t('监控失败: {v1}', v1=e))

    # 紧急清仓确认
    if st.session_state.get("confirm_emergency"):
        st.error(t("确认要清仓所有持仓吗？此操作不可撤销!"))
        col1, col2 = st.columns(2)
        with col1:
            if st.button(t("✅ 确认清仓")):
                with st.spinner(t("正在执行紧急清仓...")):
                    try:
                        we = _get_workflow_engine()
    ...
    # 实盘迁移面板
    st.subheader(t("🔴 实盘迁移管理"))
    st.caption(t("Paper Trading -> Live Trading 安全迁移 (6 项预检 + 渐进式仓位)"))
    ...
    try:
        ...
        col_m1.metric(t("当前模式"), t("📝 模拟盘") if mig_status.get("mode") == "paper" else t("🔴 实盘"))
        col_m2.metric(t("仓位系数"), f"{mig_status.get('current_scale_factor', 1.0):.0%}")
        col_m3.metric(t("迁移阶段"), mig_status.get("phase", t("未迁移")))

        # 迁移就绪检查
        with st.expander(t("🔍 迁移就绪检查"), expanded=not mig_status.get("migrated", False)):
            if st.button(t("📋 执行预检"), key="btn_migration_check"):
                with st.spinner(t("正在执行 6 项预检...")):
                    try:
            ...
            if check_result:
                ...
                if check_result.all_passed if hasattr(check_result, "all_passed") else check_result.get("all_passed"):
                    st.success(t("✅ 所有预检通过，可以安全迁移!"))
                else:
                    st.warning(t("⚠️ 部分预检未通过，请先解决以上问题"))

        # 迁移操作
        if not mig_status.get("migrated", False):
            col_mig1, col_mig2 = st.columns(2)
            with col_mig1:
                if st.button(t("🔴 迁移到实盘"), type="secondary", key="btn_migrate"):
                    st.session_state["confirm_migrate"] = True
            with col_mig2:
                st.caption(t("⚠️ 迁移需通过 6 项预检"))

            if st.session_state.get("confirm_migrate"):
                st.error(t("⚠️ 确认要迁移到实盘交易吗？此操作将切换到真实资金交易!"))
                col_c1, col_c2 = st.columns(2)
                with col_c1:
                    if st.button(t("✅ 确认迁移"), key="btn_confirm_migrate"):
                        with st.spinner(t("正在执行迁移...")):
                            try:
                                result = live_migration_manager.migrate(confirm=True)
                                if result.get("success"):
                                    st.success(t("✅ 迁移成功! 进入渐进式仓位阶段"))
                                    st.rerun()
                                else:
                                    st.error(t('迁移失败: {v1}', v1=result.get('reason', '未知')))
                            except Exception as e:
                                st.error(t('迁移失败: {v1}', v1=e))
                        st.session_state["confirm_migrate"] = False
                with col_c2:
                    if st.button(t("❌ 取消"), key="btn_cancel_migrate"):
                        st.session_state["confirm_migrate"] = False
                        st.rerun()
        else:
            st.info(t('📌 已迁移到实盘 | 迁移时间: {v1}', v1=mig_status.get('migration_date', 'N/A')[:19]))
            if st.button(t("📋 回滚到模拟盘"), type="secondary", key="btn_rollback"):
                with st.spinner(t("正在回滚...")):
                    try:
                        result = live_migration_manager.rollback()
                        if result.get("success"):
                            st.success(t("✅ 已回滚到模拟盘模式"))
                            st.rerun()
                    except Exception as e:
                        st.error(t('回滚失败: {v1}', v1=e))

    except Exception as e:
        st.info(t('实盘迁移模块未就绪: {v1}', v1=e))

    st.divider()

    # 调度器状态
    st.subheader(t("⏰ 定时任务调度器"))
    try:
        cc = _get_cloud_client()
        sched_status = cc.get_scheduler_status()
        if sched_status["running"]:
            st.success(t('调度器运行中 | {v1} 个任务', v1=sched_status['task_count']))
        else:
            st.info(t("调度器未启动"))
        ...
        col1, col2 = st.columns(2)
        with col1:
            if st.button(t("▶️ 启动调度器")) and not sched_status["running"]:
                try:
                    # 本地导入: trading_scheduler 仅在本地进程内有效 (云端模式由云端服务管理)
                    ...
                    st.error(t('启动失败: {v1}', v1=e))
        with col2:
            if st.button(t("⏹️ 停止调度器")) and sched_status["running"]:
                try:
                    from src.scheduler.scheduler import trading_scheduler
                    ...
                    st.error(t('停止失败: {v1}', v1=e))
    except Exception as e:
        st.info(t('调度器模块未就绪: {v1}', v1=e))
    
    st.divider()
    
    # 系统配置（可编辑）
    st.subheader(t("📋 系统配置"))
    st.caption(t("修改后点击底部保存按钮，配置将写入 .env 文件"))
    ...
    # ── 🔑 API 密钥 ──
    with st.expander(t("🔑 API 密钥"), expanded=False):
        st.caption(t("留空表示不修改，显示为脱敏值。输入新值将覆盖原配置。"))
        ...
        new_alpaca_key = st.text_input(
            "ALPACA_API_KEY",
            value="",
            placeholder=t('当前: {v1}', v1=alpaca_key_display) if alpaca_key_display else t("未配置"),
            help=t("Alpaca 交易 API Key"),
        )
        ...
        new_alpaca_secret = st.text_input(
            "ALPACA_SECRET_KEY",
            value="",
            placeholder=t('当前: {v1}', v1=alpaca_secret_display) if alpaca_secret_display else t("未配置"),
            help=t("Alpaca 交易 API Secret Key"),
        )
        ...
        new_finnhub_key = st.text_input(
            "FINNHUB_API_KEY",
            value="",
            placeholder=t('当前: {v1}', v1=finnhub_display) if finnhub_display else t("未配置"),
            help=t("Finnhub 行情数据 API Key"),
        )
        ...
        new_fred_key = st.text_input(
            "FRED_API_KEY",
            value="",
            placeholder=t('当前: {v1}', v1=fred_display) if fred_display else t("未配置"),
            help=t("FRED 宏观经济数据 API Key"),
        )
        ...
        new_fmp_key = st.text_input(
            "FMP_API_KEY",
            value="",
            placeholder=t('当前: {v1}', v1=fmp_display) if fmp_display else t("未配置"),
            help=t("Financial Modeling Prep 财务数据 API Key"),
        )
        if new_fmp_key.strip():
            env_updates["FMP_API_KEY"] = new_fmp_key.strip()

    # ── 🧠 LLM 智能助手配置 ──
    with st.expander(t("🧠 LLM 智能助手配置"), expanded=False):
        try:
            ...
            if llm_status["enabled"]:
                st.success(t('✅ LLM 已启用 | 默认模型: {v1} | 可用: {v2}', v1=llm_status['default_model'], v2=', '.join(llm_status['available_models'])))
            elif llm_status["total_models"] > 0:
                st.warning(t('⚠️ 已配置 {v1} 个模型但无可用项 (Key 可能未填)。 模型: {v2}', v1=llm_status['total_models'], v2=', '.join(llm_status['all_models'])))
            else:
                st.info(t("📝 未配置 LLM 模型，智能助手使用规则引擎模式。配置后可启用 AI 智能对话。"))

            st.caption(t("配置一个 OpenAI 兼容 API。支持 DeepSeek、豆包、通义千问、OpenAI 等。"))
            ...
            with col_name:
                if existing_models:
                    model_options = existing_models + [t("➕ 新建模型")]
                    model_choice = st.selectbox(t("选择模型编辑"), model_options, index=0)
                    if model_choice == t("➕ 新建模型"):
                        new_model_name = st.text_input(
                            t("新模型名称"),
                            value="",
                            placeholder=t("如: mymodel1 (英文字母数字)"),
                            help=t("模型标识名，用于区分不同模型配置"),
                        )
            ...
            llm_endpoint = st.text_input(
                ...
                placeholder=t("如: https://api.deepseek.com/v1 或 https://ark.cn-beijing.volces.com/api/v3"),
                help=t("OpenAI 兼容 API 的地址"),
            )

            # API Key (密码模式，脱敏)
            if current_entry and current_entry.is_available():
                key_placeholder = t('当前: ***{v1} (留空不修改)', v1=current_entry.api_key[-4:])
            else:
                key_placeholder = t("输入 API Key")
            llm_api_key = st.text_input(
                "API Key",
                value="",
                type="password",
                placeholder=key_placeholder,
                help=t("留空表示不修改现有 Key"),
            )
            ...
            llm_model_id = st.text_input(
                ...
                placeholder=t("如: deepseek-chat, gpt-4o-mini, doubao-seed-2-0-mini-260428"),
                help=t("API 提供商指定的模型标识"),
            )
            ...
            st.warning(t('LLM 配置加载失败: {v1}', v1=e))

    # ── 📊 交易与风控 ──
    with st.expander(t("📊 交易与风控配置"), expanded=True):
        col1, col2, col3 = st.columns(3)
        ...
        with col1:
            new_trading_mode = st.selectbox(
                ...
                format_func=lambda x: t("📝 模拟盘 (Paper)") if x == "paper" else t("🔴 实盘 (Live)"),
                help=t("交易模式: paper=模拟盘, live=实盘"),
            )
        ...
        with col3:
            new_deploy_mode = st.selectbox(
                ...
                format_func=lambda x: t("🖥️ 本地") if x == "local" else t("☁️ 云端"),
                help=t("部署模式"),
            )
        ...
        with col1:
            new_max_pos = st.number_input(
                t("MAX_POSITION_PCT (单票最大仓位)"),
                min_value=0.01, max_value=1.0,
                value=float(_config_instance.MAX_POSITION_PCT),
                step=0.05,
                format="%.2f",
                help=t("单只股票最大仓位占比 (0.30 = 30%)"),
            )
        ...
        with col3:
            new_max_dd = st.number_input(
                ...
                step=0.01,
                format="%.2f",
                help=t("允许的最大回撤比例 (0.15 = 15%)"),
            )
            if new_max_dd != _config_instance.MAX_DRAWDOWN:
                env_updates["MAX_DRAWDOWN"] = new_max_dd

    # ── 🎯 选股配置 ──
    with st.expander(t("🎯 选股配置"), expanded=False):
        col1, col2, col3 = st.columns(3)

        with col1:
            new_max_candidates = st.number_input(
                t("MAX_CANDIDATES (候选股票上限)"),
                min_value=1, max_value=50,
    ...
    with st.expander(t("🪙 加密货币配置"), expanded=False):
        ...
        with col2:
            new_crypto_trading = st.checkbox(
                t("CRYPTO_TRADING_ENABLED (启用加密货币交易)"),
        ...
        with col1:
            new_crypto_max_pos = st.number_input(
                t("CRYPTO_MAX_POSITION_PCT (单币最大仓位)"),
                min_value=0.01, max_value=1.0,
        ...
        with col1:
            new_crypto_stoploss = st.number_input(
                t("CRYPTO_SINGLE_STOP_LOSS (单币止损)"),
                min_value=0.01, max_value=0.50,
        ...
        with col2:
            new_crypto_trade_limit = st.number_input(
                t("CRYPTO_DAILY_TRADE_LIMIT (日交易次数)"),
        ...
        with col3:
            new_crypto_max_positions = st.number_input(
                t("CRYPTO_MAX_TOTAL_POSITIONS (最大持仓币种数)"),
                min_value=1, max_value=50,
    ...
    # ── ☁️ 部署与系统配置 ──
    with st.expander(t("☁️ 部署与系统配置"), expanded=False):
        col1, col2 = st.columns(2)

        with col1:
            new_qlib_enabled = st.checkbox(
                t("QLIB_ENABLED (启用 Qlib)"),
        ...
        with col1:
            new_cloud_url = st.text_input(
                t("CLOUD_API_URL (云端 API 地址)"),
                value=_config_instance.CLOUD_API_URL or "",
                help=t("本地上传模型到云端时使用的 API 地址"),
            )
    ...
    with col2:
        if st.button(t("💾 保存配置到 .env"), type="primary", width='stretch'):
            if not env_updates:
                st.info(t("ℹ️ 没有检测到配置变更"))
            else:
                try:
                    ...
                    if any(k.startswith("LLM_") for k in env_updates):
                        try:
                            ...
                            st.info(t("🧠 LLM 配置已重新加载"))
                        except Exception as llm_e:
                            st.warning(t('LLM 配置重载失败: {v1}', v1=llm_e))
                    st.info(t("💡 部分配置可能需要重启服务后生效"))
                except Exception as e:
                    st.error(t('❌ 保存失败: {v1}', v1=e))

    # 显示变更预览
    if env_updates:
        with st.expander(t('📝 待保存的变更 ({v1} 项)', v1=len(env_updates)), expanded=False):
            for key, value in env_updates.items():
...
def _ensure_chat_server():
    ...
    try:
        ...
        st.warning(t('智能助手服务启动失败: {v1}', v1=e))
        return False


def _inject_chat_widget():
    """注入悬浮聊天窗口 (v1.1.0: JS 创建元素，防止 Streamlit rerun 导致状态丢失)"""
    # i18n: 获取主系统当前语言, 传给聊天 iframe (AI助手语言跟随主系统)
    main_lang = get_language()  # "zh" / "en"
    ...
    (function() {{
        ...
        if (!doc.getElementById('chat-toggle')) {{
            ...
            win.innerHTML = '<div id="chat-loading"><div>🤖 {t("智能助手加载中")}</div>'
        ...
        // i18n: 向聊天 iframe 推送主系统语言 (AI助手语言跟随主系统)
        // 主系统语言由 Python 端 get_language() 渲染进 JS 变量
        var MAIN_LANG = '{main_lang}';
        function pushLangToChat() {{
            var iframe = doc.getElementById('chat-frame');
            if (iframe && iframe.contentWindow) {{
                try {{
                    iframe.contentWindow.postMessage({{
        ...
        // 监听聊天 iframe 发来的语言请求
        pwin._chatLangHandler = function(event) {{
            if (!event.data || event.data.type !== 'request_lang') return;
            var iframe = doc.getElementById('chat-frame');
            if (!iframe || event.source !== iframe.contentWindow) return;
            pwin.setTimeout(pushLangToChat, 100);
        }};
        pwin.addEventListener('message', pwin._chatLangHandler);
        // 首次推送语言 (延迟1.2秒等 iframe 就绪, 略晚于主题推送)
        pwin.setTimeout(pushLangToChat, 1200);
        // 语言变更后 Streamlit 会 rerun, iframe 重建时由上方定时推送兜底
        pwin.setInterval(pushLangToChat, 5000);
