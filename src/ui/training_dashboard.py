...
def _render_model_list():
    ...
    # 显示层翻译列名 (内部 key 保持中文)
    df_models_display = df_models.rename(columns={c: t(c) for c in df_models.columns if t(c) != c})
    st.dataframe(df_models_display, width='stretch', hide_index=True)
...
def _set_best_model(model_name: str, current_best: dict):
    ...
    try:
        ...
        st.success(t("✅ 已将 '{v1}' 设为最佳模型!", v1=model_name))
        st.cache_data.clear()
        st.rerun()
...
def _delete_model_file(model_name: str):
    ...
    try:
        model_path.unlink()
        st.success(t('✅ 已删除模型文件: {v1}', v1=model_name))
        st.cache_data.clear()
        st.rerun()
    except Exception as e:
...
def _render_train_single():
    ...
    if st.button(t("🚀 开始训练"), type="primary", key="btn_start_train"):
        ...
        # 执行训练
        with st.spinner(t('正在训练 {v1} 模型... (可能需要 1-5 分钟)', v1=model_type.upper())):
            try:
                ...
                if result.model_path or result.valid_ic != 0:
                    st.success(t('✅ {v1} 训练完成!', v1=model_type.upper()))
                else:
                    st.warning(t('⚠️ {v1} 训练完成，但可能未产生有效结果 (数据不足?)', v1=model_type.upper()))

            except Exception as e:
                st.error(t('训练失败: {v1}', v1=e))
                import traceback
    ...
    if result:
        ...
        col_r1.metric(t("模型类型"), result.model_type.upper())
    ...
    if st.button(t("⚔️ 开始竞赛"), type="primary", key="btn_start_competition"):
        ...
        try:
            ...
            for i, mt in enumerate(selected_models):
                status_text.info(t('正在训练 {v1} ({v2}/{v3})...', v1=mt.upper(), v2=i+1, v3=total))
                progress.progress((i / total), text=t('训练 {v1} ({v2}/{v3})', v1=mt.upper(), v2=i+1, v3=total))
                ...
                try:
                    ...
                    st.error(t('{v1} 训练失败: {v2}', v1=mt, v2=e))
...
def _render_factor_search():
    ...
    if st.button(t("🔍 开始因子搜索"), type="primary", key="btn_factor_search"):
        ...
        with st.spinner(t("正在搜索有效因子... (可能需要 1-3 分钟)")):
            try:
                ...
                if results:
                    st.success(t('✅ 找到 {v1} 个有效因子!', v1=len(results)))
                else:
                    st.warning(t("未找到符合条件的因子，尝试调整阈值"))

            except Exception as e:
                st.error(t('因子搜索失败: {v1}', v1=e))
                import traceback
                st.text(traceback.format_exc())
    ...
    if factor_results:
        ...
        if effective_factors:
            st.info(t('发现 {v1} 个有效因子，可前往「📈 策略管理」创建使用这些因子的策略', v1=len(effective_factors)))
            st.code(t('有效因子: {v1}', v1=', '.join(effective_factors[:10])))
        else:
...
def _render_training_history():
    ...
    if mlflow_runs:
        ...
        # 显示层翻译列名 (内部 key 保持中文)
        df_history_display = df_history.rename(columns={c: t(c) for c in df_history.columns if t(c) != c})
