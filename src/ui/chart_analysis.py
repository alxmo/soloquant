...
def _fetch_from_akshare(symbol: str, period: str, days_back: int = None) -> pd.DataFrame:
    ...
    try:
        ...
        st.warning(t('AKShare 数据获取失败: {v1}', v1=e))
        return pd.DataFrame()
...
def _fetch_from_yfinance(symbol: str, interval: str, days_back: int = None) -> pd.DataFrame:
    ...
    try:
        ...
        st.warning(t('yfinance 数据获取失败: {v1}', v1=e))
        return pd.DataFrame()
...
def render_kline_chart(
    ...
    # 创建子图
    subplot_titles = [t('📊 {v1} · {v2} K线', v1=symbol, v2=t(period))]
    if show_volume:
    ...
    if profile:
        ...
        with col_l:
            ...
            if profile.get('finnhubIndustry'):
                info_parts.append(t('🏷️ 行业: {v1}', v1=profile['finnhubIndustry']))
            if profile.get('country'):
                info_parts.append(t('🌍 国家: {v1}', v1=profile['country']))
            if profile.get('exchange'):
                info_parts.append(t('🏛️ 交易所: {v1}', v1=profile['exchange']))
            if profile.get('weburl'):
                info_parts.append(t('🔗 [官网]({v1})', v1=profile['weburl']))
            if profile.get('employeeTotal'):
                info_parts.append(t('👥 员工: {v1:,}', v1=profile['employeeTotal']))
            if info_parts:
...
def page_chart_analysis():
    ...
    with col_chart:
        ...
        with col_watch:
            ...
            if is_watched:
                if st.button(t("💔 取消关注"), key=f"btn_unwatch_chart_{selected_symbol}", width='stretch'):
                    user_wl.remove(selected_symbol)
                    _save_user_watchlist(user_wl)
                    st.success(t('已取消关注 {v1}', v1=selected_symbol))
                    st.rerun()
            else:
                if st.button(t("⭐ 关注该股票"), key=f"btn_watch_chart_{selected_symbol}", width='stretch'):
                    if selected_symbol not in user_wl:
                        user_wl.append(selected_symbol)
                        _save_user_watchlist(user_wl)
                    st.success(t('已关注 {v1}', v1=selected_symbol))
                    st.rerun()
            # 显示关注列表数量
        ...
        with col_strat:
            ...
            with st.expander(t("📈 为该股票生成交易策略"), expanded=False):
                default_name = t('行情策略_{v1}_{v2}', v1=selected_symbol, v2=datetime.now().strftime('%Y%m%d'))
                ...
                with col_s1:
                    model_type = st.selectbox(
                        t("模型类型"), ["lightgbm", "lstm"], key=f"chart_strat_model_{selected_symbol}"
                    )
                with col_s2:
                ...
                if st.button(t("🚀 创建策略"), type="primary",
                             key=f"chart_btn_create_strat_{selected_symbol}", width='stretch'):
                    try:
                        ...
                        st.info(t('💡 股票池: {v1} | 模型: {v2} | 仓位: {v3:.0%}', v1=selected_symbol, v2=strategy.model_type, v3=max_position))
        ...
        with st.spinner(t('加载 {v1} {v2} K线数据...', v1=selected_symbol, v2=period)):
            df = fetch_kline_data(selected_symbol, period, days_back)

        if df is None or len(df) == 0:
            st.error(t('❌ 无法获取 {v1} 的K线数据，请检查股票代码或网络连接', v1=selected_symbol))
            st.info(t("💡 提示: 日内分时数据需要安装 yfinance (`pip install yfinance`)"))
            return
        ...
        # ─── K线数据表格 (可折叠) ───
        with st.expander(t('📋 {v1} {v2} 原始数据 (最近20条)', v1=selected_symbol, v2=t(period)), expanded=False):
